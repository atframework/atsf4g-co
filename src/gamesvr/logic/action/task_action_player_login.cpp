...
task_action_player_login::task_action_player_login(dispatcher_start_data_t &&param)
...
int task_action_player_login::operator()() {
  ...
  if (user && user->get_login_info().login_code() == msg_body.login_code() &&
    ...
    FWPLOGDEBUG(*user, "relogin using login code: {}", msg_body.login_code());
  ...
  user = player_manager::me()->create_as<player>(msg_body.user_id(), zone_id, msg_body.open_id(), tb, version);
...
int task_action_player_login::on_success() {
  ...
  player::ptr_t user = player_manager::me()->find_as<player>(req.body().mcs_login_req().user_id(), get_zone_id());
  if (!user) {
  ...
  if (user->get_session() != s) {
    ...
    set_rsp_code(hello::EN_ERR_LOGIN_OTHER_DEVICE);
    return get_ret_code();
  }
...
int task_action_player_login::on_failed() {
  ...
  FWLOGERROR("session [{},{}] login failed, rsp code: {}, ret code: {}", get_gateway_info().first,
             get_gateway_info().second, get_rsp_code(), get_ret_code());
