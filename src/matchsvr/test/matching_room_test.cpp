...
CASE_TEST(matchsvr_matching_room, maintains_level_intersection_and_selected_level) {
  ...
  CASE_EXPECT_EQ(2, room.get_units().at(1).acceptable_level_ids_size());
  ...
  CASE_EXPECT_FALSE(room.add_unit(disjoint));
...
CASE_TEST(matchsvr_matching_room, finalizes_battle_faction_ids_from_membership_once) {
  auto room = make_room();
  CASE_EXPECT_TRUE(room.add_unit(make_unit(10, 10010, 1)));
  CASE_EXPECT_TRUE(room.add_unit(make_unit(11, 10011, 1)));
  CASE_EXPECT_TRUE(room.add_unit(make_unit(20, 10020, 1)));
  google::protobuf::RepeatedPtrField<PROJECT_NAMESPACE_ID::DMatchingFactionAssignment> assignments;
