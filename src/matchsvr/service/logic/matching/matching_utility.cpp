...
bool same_user(const PROJECT_NAMESPACE_ID::DUserIDKey& left,
...
bool unit_has_player(const PROJECT_NAMESPACE_ID::DMatchingUnit& unit,
  ...
  return std::any_of(unit.players().begin(), unit.players().end(),
                     [&expected](const auto& player) { return same_user(player.user_key(), expected); });
}

bool contains_user(const google::protobuf::RepeatedPtrField<PROJECT_NAMESPACE_ID::DUserIDKey>& users,
                   const PROJECT_NAMESPACE_ID::DUserIDKey& expected) noexcept {
  return std::any_of(users.begin(), users.end(),
                     [&expected](const auto& value) { return same_user(value, expected); });
}
