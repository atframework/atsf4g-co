...
EXCEL_CONFIG_LOADER_API int config_manager::init_new_group() {
  ...
  do {
    // 检查版本号
    ${spin_lock_namespace}::lock::read_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock> wlh;
    if (enable_multithread_lock_) {
      wlh = ${spin_lock_namespace}::lock::read_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock>{config_group_lock_};
    }
...
% endfor
  ...
  if (ret >= 0) {
    ...
    if (enable_multithread_lock_) {
      wlh = ${spin_lock_namespace}::lock::write_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock>{config_group_lock_};
    }
    ++ reload_version_;
...
EXCEL_CONFIG_LOADER_API void config_manager::reset() {
...

  {
    ${spin_lock_namespace}::lock::write_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock> wlh;
    if (enable_multithread_lock_) {
      wlh = ${spin_lock_namespace}::lock::write_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock>{handle_lock_};
    }
    max_group_number_ = 8;
    override_same_version_ = false;
...
EXCEL_CONFIG_LOADER_API const config_manager::config_group_ptr_t& config_manager::get_current_config_group() {
...

  {
    ${spin_lock_namespace}::lock::read_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock> rlh;
    if (enable_multithread_lock_) {
      rlh = ${spin_lock_namespace}::lock::read_lock_holder<${spin_lock_namespace}::lock::spin_rw_lock>{config_group_lock_};
    }
    if (!config_group_list_.empty()) {
