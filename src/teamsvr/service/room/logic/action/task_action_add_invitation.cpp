...
ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API task_action_add_invitation::task_action_add_invitation(
    dispatcher_start_data_type&& param)
    : base_type(std::move(param)) {}

ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API task_action_add_invitation::~task_action_add_invitation() {}

ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API const char* task_action_add_invitation::name() const {
  return "task_action_add_invitation";
}

ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API task_action_add_invitation::result_type task_action_add_invitation::operator()() {
  ...
  if (!req_body.has_invitation()) {
    set_response_code(PROJECT_NAMESPACE_ID::EN_ERR_INVALID_PARAM);
    TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
  }

  // 按队伍一致性哈希路由到 teamsvr-room 节点，不在本节点则转发
  uint64_t dest_server_id = rpc::team::team_api::get_teamsvr_room_server_id_of_zone(req_body.invitation().team_key());
  if (0 == dest_server_id) {
    FCTXLOGERROR(get_shared_context(), "no ready teamsvr-room node for team {}:{}",
                 req_body.invitation().team_key().zone_id(), req_body.invitation().team_key().team_id());
  ...
  if (dest_server_id != logic_config::me()->get_local_server_id()) {
    ...
    if (0 != forward_ret || !forward_ok) {
      FCTXLOGERROR(get_shared_context(), "forward team {}:{} add_invitation to dest server {} failed! ret:{} ok:{}",
                   req_body.invitation().team_key().zone_id(), req_body.invitation().team_key().team_id(), dest_server_id, forward_ret, forward_ok ? 1 : 0);
  ...
  auto room = team_room_manager::me()->mutable_room(get_shared_context(), req_body.invitation().team_key());
  if (!room) {
    set_response_code(PROJECT_NAMESPACE_ID::EN_ERR_DTMQ_SERVICE_NOT_AVAILABLE);
    TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
  }
...
ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API int task_action_add_invitation::on_success() { return get_result(); }

ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API int task_action_add_invitation::on_failed() { return get_result(); }
