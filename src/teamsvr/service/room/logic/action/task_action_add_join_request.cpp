...
ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API const char* task_action_add_join_request::name() const {
  return "task_action_add_join_request";
}
...
task_action_add_join_request::operator()() {
  ...
  uint64_t dest_server_id = rpc::team::team_api::get_teamsvr_room_server_id_of_zone(req_body.join_request().team_key());
  if (0 == dest_server_id) {
    FCTXLOGERROR(get_shared_context(), "no ready teamsvr-room node for team {}:{}",
                 req_body.join_request().team_key().zone_id(), req_body.join_request().team_key().team_id());
  ...
  if (dest_server_id != logic_config::me()->get_local_server_id()) {
    ...
    if (0 != forward_ret || !forward_ok) {
      FCTXLOGERROR(get_shared_context(), "forward team {}:{} add_join_request to dest server {} failed! ret:{} ok:{}",
                   req_body.join_request().team_key().zone_id(), req_body.join_request().team_key().team_id(), dest_server_id, forward_ret, forward_ok ? 1 : 0);
  ...
  auto room = team_room_manager::me()->mutable_room(get_shared_context(), req_body.join_request().team_key());
  if (!room) {
    set_response_code(PROJECT_NAMESPACE_ID::EN_ERR_DTMQ_SERVICE_NOT_AVAILABLE);
    TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
  }
...
ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API int task_action_add_join_request::on_success() { return get_result(); }

ATFRAMEWORK_TEAM_TEAMROOMSERVICE_API int task_action_add_join_request::on_failed() { return get_result(); }
