...
static rpc::result_code_type save_user_data(rpc::context& ctx, router_user_cache::ptr_t& cache, size_t batch_job_number,
  ...
  if (batch_job_number > 0) {
    ...
    if (ret < 0) {
      ...
      if (PROJECT_NAMESPACE_ID::err::EN_ROUTER_NOT_WRITABLE == ret) {
        FWLOGWARNING("save user {}({}:{}) failed, res: {}({})", openid, zone_id, user_id, ret,
                     protobuf_mini_dumper_get_error_msg(ret));
      } else {
        FWLOGERROR("save user {}({}:{}) failed, res: {}({})", openid, zone_id, user_id, ret,
                   protobuf_mini_dumper_get_error_msg(ret));
      }
      RPC_RETURN_CODE(ret);
    }
  ...
  if (!complete_jobs_idx.empty()) {
    ...
    if (ret < 0) {
      FWLOGERROR("delete async jobs for user {}({}:{}) failed, res: {}({})", openid, zone_id, user_id, ret,
                 protobuf_mini_dumper_get_error_msg(ret));
      RPC_RETURN_CODE(ret);
    }
...
task_action_user_remote_patch_jobs::result_type task_action_user_remote_patch_jobs::operator()() {
  need_restart_ = false;
  is_writable_ = false;
  if (!param_.user_inst) {
  ...
  // 缓存已被踢出，当前user是路由缓存，忽略patch
  if (!cache->is_writable()) {
    TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
  }

  // 注意这里会续期缓存生命周期，所以要确保前面判定都过后才能到这里
  if (!cache->is_object_equal(param_.user_inst)) {
    TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
  }
  is_writable_ = true;
  need_restart_ = false;
  ...
  const ::google::protobuf::EnumDescriptor* desc = PROJECT_NAMESPACE_ID::EnUserAsyncJobsType_descriptor();
  for (int pull_jobs_idx = 0; desc && is_writable_ && !pending_to_logout && pull_jobs_idx < desc->value_count() &&
                              param_.user_inst->is_inited() && false == need_restart_;
       ++pull_jobs_idx) {
    ...
    complete_jobs_idx.reserve(PROJECT_NAMESPACE_ID::EN_SL_USER_ASYNC_JOBS_BATCH_NUMBER);
    ...
    for (size_t i = 0; i < job_list.size() && !pending_to_logout && false == need_restart_; ++i) {
      ...
      if (param_.user_inst->get_user_async_jobs_manager().is_job_uuid_exists(
    ...
    // 再执行retry队列
    for (auto& job_data_ptr : param_.user_inst->get_user_async_jobs_manager().get_retry_jobs(val_desc->number())) {
    ...
    // 如果用户离线和正在准备登出则停止异步任务流程，下次登入再继续
    {
      session::ptr_t s = param_.user_inst->get_session();
      if (!s) {
  ...
  // 如果是执行过程中用户对象离线导致不可写，直接跳过即可，前面会打印warning日志，不需要输出错误
  if (PROJECT_NAMESPACE_ID::err::EN_ROUTER_NOT_WRITABLE == ret) {
    ret = 0;
  }
...
int task_action_user_remote_patch_jobs::on_success() {
  ...
  // 尝试再启动一次，启动排队后的任务
  if (is_writable_ && param_.user_inst) {
    if (task_type_trait::get_task_id(param_.user_inst->get_user_async_jobs_manager().remote_command_patch_task_) ==
        get_task_id()) {
      task_type_trait::reset_task(param_.user_inst->get_user_async_jobs_manager().remote_command_patch_task_);
    }

    if (need_restart_) {
      param_.user_inst->get_user_async_jobs_manager().reset_async_jobs_protect();
    }
    ...
    new_ctx.setup_tracer(new_tracer, "task_action_user_remote_patch_jobs.on_success", std::move(trace_start_option));
    param_.user_inst->get_user_async_jobs_manager().try_async_jobs(new_ctx);
  ...
  if (patched_job_number_ > 0) {
    // 数据变更推送
    param_.user_inst->send_all_syn_msg(get_shared_context());
  }
...
int task_action_user_remote_patch_jobs::on_failed() {
  ...
  if (is_writable_ && param_.user_inst) {
    ...
    new_ctx.setup_tracer(new_tracer, "task_action_user_remote_patch_jobs.on_failed", std::move(trace_start_option));
    param_.user_inst->get_user_async_jobs_manager().try_async_jobs(new_ctx);
...
int32_t task_action_user_remote_patch_jobs::do_job(int32_t job_type, const async_job_ptr_type& job_data) {
  ...
  if (iter_async != async_callbacks.end() && iter_async->second) {
    ...
    if (nullptr == fds) {
      sub_job_name = atfw::util::log::format("task_action_user_remote_patch_jobs.{}",
                                             static_cast<int32_t>(job_data->action_case()));
    } else {
      sub_job_name = atfw::util::log::format("task_action_user_remote_patch_jobs.{}", fds->name());
    }
    auto fn = iter_async->second;
    auto user_ptr = param_.user_inst;
...
void task_action_user_remote_patch_jobs::append_sub_task(task_type_trait::task_type task_inst) {
