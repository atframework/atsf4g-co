...
class task_action_rank_update_settlement : public task_action_no_req_base {
 ...
 private:
  ...
  // 返回是否需要重试失败或执行下一轮
  ATFW_EXPLICIT_NODISCARD_ATTR rpc::result_code_type settle_rank_once(
  ...
  // 返回是否需要执行下一轮
  ATFW_EXPLICIT_NODISCARD_ATTR rpc::result_code_type cleanup_save(
  ...
  ATFW_EXPLICIT_NODISCARD_ATTR rpc::result_code_type process_rank(
      rpc::context& ctx, bool& allow_continue,
      const ::excel::excel_config_type_traits::shared_ptr<excel::config_group_t>& group,
