...
#include <protocol/config/friend_recommend.config.pb.h>
...
#include "app/handle_ss_rpc_friendrecommendservice.atfw.gen.h"
...
class main_service_module : public atfw::atapp::module_impl {
 public:
  int init() override {
    ...
    // register handles
    INIT_CALL_FN(handle::friend_api::register_handles_for_friendrecommendservice);
    return 0;
  };
...
int main(int argc, char *argv[]) {
  ...
  logic_config::me()->set_server_instance_config_loader(
      [](atfw::atapp::app &app_, logic_config & /*cfg*/, logic_config::server_instance_config_ptr &to) {
        auto config_ptr =
            atfw::component::memory::stl::make_strong_rc<atframework::friend_api::config::friendsvr_recommend_cfg>();
        app_.parse_configures_into(*config_ptr, "friendsvr_recommend", "ATAPP_FRIENDSVR_RECOMMEND");
