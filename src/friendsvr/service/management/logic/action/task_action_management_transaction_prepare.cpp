...
#include "logic/action/task_action_management_transaction_prepare.h"
...
ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API const char* task_action_management_transaction_prepare::name()
    const {
  return "task_action_management_transaction_prepare";
}
...
task_action_management_transaction_prepare::operator()() {
  ...
  if (0 == user_id || 0 == zone_id) {
    FWLOGERROR("try to find router cache id from request {} for {} and player {}:{} failed.",
               "SSFriendTransactionPrepareReq", name(), zone_id, user_id);
...
ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API int task_action_management_transaction_prepare::on_success() {
  return get_result();
}

ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API int task_action_management_transaction_prepare::on_failed() {
  return get_result();
}
