...
#include "logic/action/task_action_management_transaction_commit.h"
...
task_action_management_transaction_commit::task_action_management_transaction_commit(dispatcher_start_data_type&& param)
    : base_type(std::move(param)) {}

ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API
task_action_management_transaction_commit::~task_action_management_transaction_commit() {}

ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API const char* task_action_management_transaction_commit::name() const {
  return "task_action_management_transaction_commit";
}

ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API task_action_management_transaction_commit::result_type
task_action_management_transaction_commit::operator()() {
  ...
  if (0 == user_id || 0 == zone_id) {
    FWLOGERROR("try to find router cache id from request {} for {} and player {}:{} failed.",
               "SSFriendTransactionCommitReq", name(), zone_id, user_id);
  ...
  set_response_code(RPC_AWAIT_CODE_RESULT(friend_obj->get_transaction_handle().commit(
...
ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API int task_action_management_transaction_commit::on_success() {
  return get_result();
}

ATFRAMEWORK_FRIEND_API_FRIENDMANAGEMENTSERVICE_API int task_action_management_transaction_commit::on_failed() {
  return get_result();
}
