...
SERVER_FRAME_API void logic_hpa_controller::reload() {
  ...
  // Patch补充HPA策略路由配置
  auto& ready_rules =
      *logic_config::me()->mutable_logic_cfg()->mutable_hpa()->mutable_discovery()->mutable_scaling_ready();
  auto& target_rules =
      *logic_config::me()->mutable_logic_cfg()->mutable_hpa()->mutable_discovery()->mutable_scaling_target();
  protobuf_copy_message(ready_rules, logic_config::me()->get_logic_cfg().discovery_selector());
...
util::nostd::nonnull<std::shared_ptr<logic_hpa_policy>> logic_hpa_controller::internal_create_hpa_policy(
  ...
  int64_t pull_default_time_range =
      logic_config::me()->get_logic_cfg().hpa().metrics().pull_default_time_range_multiplying_factor();
...
bool logic_hpa_controller::is_main_hpa_controller() const noexcept {
  ...
  time_t pull_interval_sec = logic_config::me()->get_logic_cfg().hpa().metrics().pull_interval().seconds();
  if (pull_interval_sec <= 0) {
    pull_interval_sec = 60;
  }
