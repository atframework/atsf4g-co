...
SERVER_FRAME_API const atfw::atapp::protocol::atapp_metadata* logic_hpa_discovery_select(
  ...
  static_assert(std::is_same<decltype(logic_config::me()->get_logic_cfg().hpa().discovery().scaling_ready()),
                             decltype(logic_config::me()->get_logic_cfg().hpa().discovery().scaling_target())>::value,
                "Type checking - 1");
  static_assert(std::is_same<decltype(logic_config::me()->get_logic_cfg().hpa().discovery().scaling_ready()),
                             decltype(logic_config::me()->get_logic_cfg().discovery_selector())>::value,
                "Type checking - 2");
  ...
  switch (mode) {
    case logic_hpa_discovery_select_mode::kTarget:
      use_cfg = &logic_config::me()->get_logic_cfg().hpa().discovery().scaling_target();
      break;
    default:
      use_cfg = &logic_config::me()->get_logic_cfg().hpa().discovery().scaling_ready();
      break;
  }
