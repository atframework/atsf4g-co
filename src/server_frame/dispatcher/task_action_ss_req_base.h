...
public:
    task_action_ss_rpc_base(dispatcher_start_data_t COPP_MACRO_RV_REF start_param): base_type(COPP_MACRO_STD_MOVE(start_param)), has_unpack_request_(false), has_pack_response_(false) { }

    inline rpc_request_type& get_request_body() {
    ...
    inline const rpc_response_type& get_response_body() const { return response_body_; }
    inline rpc_response_type& get_response_body() { return response_body_; }

    static const std::string& get_request_type_url() {
        return rpc_request_type::descriptor()->full_name();
    }

    static const std::string& get_response_type_url() {
...
private:
    void unpack_request() {
        ...
        if (get_request().head().has_rpc_request()) {
            if (get_request_type_url() != get_request().head().rpc_request().type_url()) {
                FWLOGERROR("task {} [{}] except message {}, real got {}", name(), get_task_id_llu(),
                    get_request_type_url().c_str(), get_request().head().rpc_request().type_url().c_str()
                );
            }
        } else if (get_request().head().has_rpc_stream()) {
            if (get_request_type_url() != get_request().head().rpc_stream().type_url()) {
                FWLOGERROR("task {} [{}] except message {}, real got {}", name(), get_task_id_llu(),
                    get_request_type_url().c_str(), get_request().head().rpc_stream().type_url().c_str()
                );
            }
        }

        if (false == request_body_.ParseFromString(get_request().body_bin())) {
            FWLOGERROR("task {} [{}] try to parse message {} failed, msg: {}", name(), get_task_id_llu(),
                get_request_type_url().c_str(), request_body_.InitializationErrorString().c_str()
            );
        } else {
            FWLOGDEBUG("task {} [{}] parse rpc request message {} success:\n{}", name(), get_task_id_llu(),
                get_request_type_url().c_str(), protobuf_mini_dumper_get_readable(request_body_)
            );
        }
    }
    ...
    void pack_response() {
        ...
        if (false == response_body_.SerializeToString(rsp.mutable_body_bin())) {
            FWLOGERROR("task {} [{}] try to serialize message {} failed, msg: {}", name(), get_task_id_llu(),
                get_response_type_url().c_str(), response_body_.InitializationErrorString().c_str()
            );
        } else {
            FWLOGDEBUG("task {} [{}] serialize rpc response message {} success:\n{}", name(), get_task_id_llu(),
                get_response_type_url().c_str(), protobuf_mini_dumper_get_readable(response_body_)
            );
        }
    }
