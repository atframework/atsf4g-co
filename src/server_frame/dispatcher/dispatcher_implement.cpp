...
int32_t dispatcher_implement::on_receive_message(rpc::context &ctx, msg_raw_t &msg, void *priv_data, uint64_t sequence) {
    ...
    if (get_instance_ident() != msg.msg_type) {
        FWLOGERROR("msg.msg_type expected: {}, real: {}.", get_instance_ident_llu(), msg.msg_type);
        return hello::err::EN_SYS_PARAM;
    }
    ...
    if (hello::EN_MSG_OP_TYPE_MIXUP == op_type || hello::EN_MSG_OP_TYPE_UNARY_RESPONSE == op_type) {
        ...
        if (hello::EN_MSG_OP_TYPE_UNARY_RESPONSE == op_type) {
            FWLOGDEBUG("Ignore response message {}:{} of {} without task id", pick_rpc_name(msg), pick_msg_type_id(msg), name());
            return 0;
        }
    }
...
int32_t dispatcher_implement::on_send_message_failed(rpc::context &ctx, msg_raw_t &msg, int32_t error_code, uint64_t sequence) {
    ...
    uint64_t task_id = pick_msg_task_id(msg);
    if (task_id > 0) { // 如果是恢复任务则尝试切回协程任务
        FWLOGERROR("dispatcher {} send data failed with error code = {}, try to resume task {}", name(), error_code, task_id);
        // 查找并恢复已有task
        return task_manager::me()->resume_task(task_id, callback_data);
    }
