...
template <>
UTIL_FORCEINLINE dispatcher_raw_message dispatcher_make_default<dispatcher_raw_message>() noexcept {
  return dispatcher_raw_message();
}

struct UTIL_SYMBOL_VISIBLE dispatcher_resume_data_type {
...
template <>
UTIL_FORCEINLINE dispatcher_start_data_type dispatcher_make_default<dispatcher_start_data_type>() noexcept {
  return dispatcher_start_data_type();
}

template <>
UTIL_FORCEINLINE dispatcher_resume_data_type dispatcher_make_default<dispatcher_resume_data_type>() noexcept {
...
UTIL_FORCEINLINE bool operator==(const dispatcher_await_options &l, const dispatcher_await_options &r) noexcept {
  return l.sequence == r.sequence;
}

namespace std {
template <>
struct UTIL_SYMBOL_VISIBLE hash<dispatcher_await_options> {
  size_t operator()(const dispatcher_await_options &value) const noexcept { return hash<uint64_t>()(value.sequence); }
};
}  // namespace std

template <>
UTIL_FORCEINLINE dispatcher_await_options dispatcher_make_default<dispatcher_await_options>() noexcept {
