...
SERVER_FRAME_API void report_shared_message_defer_after_moved(const std::string &demangle_name) {
  auto object_full_name = util::log::format("rpc::shared_message<{}>", demangle_name);
  auto message = util::log::format("{} should not be defered after moved", object_full_name);
  FWLOGERROR("{}", message);
  ...
  rpc::telemetry::opentelemetry_utility::send_notification_event(
      ...
      util::log::format("rpc.shared_message.defer_after_moved.{}", object_full_name), message,
