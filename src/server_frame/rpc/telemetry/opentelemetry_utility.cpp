...
SERVER_FRAME_API int opentelemetry_utility::tick() {
  ...
  int32_t max_record_per_loop = logic_config::me()->get_logic_cfg().telemetry().executor().max_metric_record_per_loop();
...
SERVER_FRAME_API bool opentelemetry_utility::add_global_metics_observable_int64(
  ...
  handle->collect_interval = protobuf_to_chrono_duration<>(
      logic_config::me()->get_logic_cfg().telemetry().opentelemetry().metrics().reader().export_interval());
