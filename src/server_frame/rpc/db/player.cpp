...
result_t get_all(::rpc::context &ctx, uint64_t user_id, uint32_t zone_id, hello::table_user &rsp,
                 std::string &version) {
  ...
  if (!task) {
    ...
    return ::rpc::db::result_t(__tracer.return_code(hello::err::EN_SYS_RPC_NO_TASK));
  }
  ...
  if (user_key_len <= 0) {
    ...
    return ::rpc::db::result_t(__tracer.return_code(hello::err::EN_DB_SEND_FAILED));
  }
  ...
  if (!msg.has_user() || msg.version().empty()) {
    return ::rpc::db::result_t(__tracer.return_code(hello::err::EN_DB_RECORD_NOT_FOUND));
  }
  ...
  return ::rpc::db::result_t(__tracer.return_code(hello::err::EN_SUCCESS));
}
...
result_t set(::rpc::context &ctx, uint64_t user_id, uint32_t zone_id, hello::table_user &store, std::string &version) {
  ...
  if (nullptr == reflect) {
    ...
    return ::rpc::db::result_t(__tracer.return_code(hello::err::EN_SYS_PACK));
  }
