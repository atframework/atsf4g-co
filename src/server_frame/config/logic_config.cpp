...
void logic_config::_load_db_hosts(PROJECT_SERVER_FRAME_NAMESPACE_ID::config::db_group_cfg &out) {
  for (int i = 0; i < out.host_size(); ++i) {
    ...
    if (std::string::npos == fn) {
      PROJECT_SERVER_FRAME_NAMESPACE_ID::config::db_group_gateway_cfg *db_gateway = out.add_gateways();
...
const PROJECT_SERVER_FRAME_NAMESPACE_ID::DConstSettingsType &logic_config::get_const_settings() {
  ...
  if (nullptr != desc && desc->options().HasExtension(PROJECT_SERVER_FRAME_NAMESPACE_ID::CONST_SETTINGS)) {
    const_settings_ = &desc->options().GetExtension(PROJECT_SERVER_FRAME_NAMESPACE_ID::CONST_SETTINGS);
  }

  if (nullptr == const_settings_) {
    return PROJECT_SERVER_FRAME_NAMESPACE_ID::DConstSettingsType::default_instance();
  }
