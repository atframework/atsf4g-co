...
class logic_rank_handle_key {
 public:
  RANK_SDK_API explicit logic_rank_handle_key(const PROJECT_NAMESPACE_ID::config::ExcelRankRule& rule);
  RANK_SDK_API explicit logic_rank_handle_key(const PROJECT_NAMESPACE_ID::config::ExcelRankRule& rule,
                                              uint32_t auto_reward_rank_type);
  RANK_SDK_API explicit logic_rank_handle_key(uint32_t rank_type, uint32_t instance_id, uint32_t sub_rank_type,
                                              uint32_t sub_instance_id);

  RANK_SDK_API inline logic_rank_handle_key(const logic_rank_handle_key&) = default;
  RANK_SDK_API inline logic_rank_handle_key(logic_rank_handle_key&&) = default;
  RANK_SDK_API inline logic_rank_handle_key& operator=(const logic_rank_handle_key&) = default;
  RANK_SDK_API inline logic_rank_handle_key& operator=(logic_rank_handle_key&&) = default;
...
struct logic_rank_handle_key_hash {
  RANK_SDK_API inline logic_rank_handle_key_hash() = default;
  RANK_SDK_API inline logic_rank_handle_key_hash(const logic_rank_handle_key_hash&) = default;
  RANK_SDK_API inline logic_rank_handle_key_hash(logic_rank_handle_key_hash&&) = default;
  RANK_SDK_API inline logic_rank_handle_key_hash& operator=(const logic_rank_handle_key_hash&) = default;
  RANK_SDK_API inline logic_rank_handle_key_hash& operator=(logic_rank_handle_key_hash&&) = default;

  RANK_SDK_API size_t operator()(const logic_rank_handle_key& key) const noexcept;
};
...
class logic_rank_handle_decl {
 ...
 public:
  RANK_SDK_API logic_rank_handle_decl();
  RANK_SDK_API virtual ~logic_rank_handle_decl();

  RANK_SDK_API void reset_cursor_front() noexcept;

  RANK_SDK_API void reset_cursor_back() noexcept;

  RANK_SDK_API bool next_cursor() noexcept;

  RANK_SDK_API bool previous_cursor() noexcept;

  RANK_SDK_API bool valid_cursor() const noexcept;

  RANK_SDK_API gsl::string_view get_current_open_id() const noexcept;

  RANK_SDK_API uint32_t get_current_user_zone_id() const noexcept;

  RANK_SDK_API uint32_t get_current_score() const noexcept;

  RANK_SDK_API uint32_t get_current_no() const noexcept;
  ...
  RANK_SDK_API int64_t get_current_timestamp() const noexcept;

  RANK_SDK_API uint32_t get_current_total_count() const noexcept;

  RANK_SDK_API uint32_t get_current_count() const noexcept;

  RANK_SDK_API const logic_rank_handle_data* get_current_cursor() const noexcept;

  RANK_SDK_API gsl::span<logic_rank_handle_data> get_current_span() noexcept;

  RANK_SDK_API gsl::span<const logic_rank_handle_data> get_current_span() const noexcept;
...
class logic_rank_handle_self_impl : public logic_rank_handle_decl {
 public:
  RANK_SDK_API logic_rank_handle_self_impl(uint32_t world_id, uint32_t zone_id);
  RANK_SDK_API virtual ~logic_rank_handle_self_impl();

  RANK_SDK_API uint32_t get_world_id() const noexcept override;
  RANK_SDK_API uint32_t get_zone_id() const noexcept override;

  RANK_SDK_API gsl::span<const uint32_t> get_current_sort_fields() const noexcept override;

  RANK_SDK_API gsl::span<const uint64_t> get_current_ext_fields() const noexcept override;

  RANK_SDK_API bool is_service_available() const noexcept override;

  RANK_SDK_API bool is_current(const PROJECT_NAMESPACE_ID::config::ExcelRankRule& cfg, time_t now,
                               const logic_rank_handle_key& key) const noexcept override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> get_top_rank(
      rpc::context& ctx, const logic_rank_handle_key& key, uint32_t start, uint32_t count,
      PROJECT_NAMESPACE_ID::DRankImageData* image = nullptr) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> upload_score(
      rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
      const rank_callback_private_data& callback_data,
      logic_rank_user_extend_span user_extend_data = {}) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> clear_all(
      rpc::context& ctx, const logic_rank_handle_key& key,
      PROJECT_NAMESPACE_ID::DRankImageData* image = nullptr) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> clear_special_one(
      rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid,
      PROJECT_NAMESPACE_ID::DRankImageData* image = nullptr) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> increase_score(
      rpc::context&, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
      const rank_callback_private_data& callback_data,
      logic_rank_user_extend_span user_extend_data = {}) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> decrease_score(
      rpc::context&, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
      const rank_callback_private_data& callback_data,
      logic_rank_user_extend_span user_extend_data = {}) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> get_special_one(
      rpc::context&, logic_rank_handle_data& output, const logic_rank_handle_key& key, gsl::string_view openid,
      uint32_t up_count = 0, uint32_t down_count = 0, PROJECT_NAMESPACE_ID::DRankImageData* image = nullptr) override;

  EXPLICIT_NODISCARD_ATTR RANK_SDK_API rpc::rpc_result<rank_ret_t> get_special_score(
      rpc::context&, logic_rank_handle_data& output, const logic_rank_handle_key& key, uint32_t score,
      uint32_t up_count = 0, uint32_t down_count = 0, PROJECT_NAMESPACE_ID::DRankImageData* image = nullptr) override;
...
class logic_rank_handle_variant {
 ...
 public:
  RANK_SDK_API logic_rank_handle_variant(uint32_t world_id, uint32_t zone_id,
                                         const PROJECT_NAMESPACE_ID::config::ExcelRankRule& rule);

  RANK_SDK_API logic_rank_handle_variant(const logic_rank_handle_variant& other);
  RANK_SDK_API logic_rank_handle_variant& operator=(const logic_rank_handle_variant& other);

  RANK_SDK_API logic_rank_handle_variant(logic_rank_handle_variant&& other);
  RANK_SDK_API logic_rank_handle_variant& operator=(logic_rank_handle_variant&& other);

  RANK_SDK_API virtual ~logic_rank_handle_variant();

  void init_delegate(uint32_t world_id, uint32_t zone_id);
  void destructor_delegate();

  RANK_SDK_API uint32_t get_world_id() const noexcept;
  RANK_SDK_API uint32_t get_zone_id() const noexcept;

  RANK_SDK_API gsl::span<const uint32_t> get_current_sort_fields() const noexcept;

  RANK_SDK_API gsl::span<const uint64_t> get_current_ext_fields() const noexcept;

  RANK_SDK_API bool is_service_available() const noexcept;
