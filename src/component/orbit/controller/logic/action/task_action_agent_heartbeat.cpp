...
#include "logic/action/task_action_agent_heartbeat.h"
...
task_action_agent_heartbeat::task_action_agent_heartbeat(dispatcher_start_data_type&& param)
    : base_type(std::move(param)) {}

task_action_agent_heartbeat::~task_action_agent_heartbeat() {}

const char* task_action_agent_heartbeat::name() const { return "task_action_agent_heartbeat"; }
...
task_action_agent_heartbeat::result_type task_action_agent_heartbeat::operator()() {
  ...
  int32_t rpc_result = orbit_controller_manager::me()->handle_agent_heartbeat(get_shared_context(), req_body);
