...
ATFW_UTIL_DESIGN_PATTERN_SINGLETON_EXPORT_DATA_DEFINITION(OrbitRPCDispatcher);
#  else
ATFW_UTIL_DESIGN_PATTERN_SINGLETON_IMPORT_DATA_DEFINITION(OrbitRPCDispatcher);
#  endif
#else
ATFW_UTIL_DESIGN_PATTERN_SINGLETON_VISIBLE_DATA_DEFINITION(OrbitRPCDispatcher);
#endif

ORBIT_CLIENT_SDK_API OrbitRPCDispatcher::OrbitRPCDispatcher() {}
ORBIT_CLIENT_SDK_API OrbitRPCDispatcher::~OrbitRPCDispatcher() {}

ORBIT_CLIENT_SDK_API void OrbitRPCDispatcher::init() {
...
ORBIT_CLIENT_SDK_API void OrbitRPCDispatcher::tick() { rsp_callback_execute(); }

ORBIT_CLIENT_SDK_API bool OrbitRPCDispatcher::check_rpc_success() {
...
ORBIT_CLIENT_SDK_API int32_t OrbitRPCDispatcher::dispatch(const std::string& message) {
  ::atframework::orbit::OrbitRpcMessage orbit_msg;
  if (!orbit_msg.ParseFromString(message)) {
    ...
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_PARSE_MESSAGE_FAILED;
  }
  if (!orbit_msg.has_head()) {
    OrbitClientRuntime::me()->log(OrbitClientLogLevel::kError, __FILE__, __LINE__,
                                  LOG_WRAPPER_FWAPI_FORMAT("[ORBIT_RPC] message head not found.\n{}", message));
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_MESSAGE_HEAD_NOT_FOUND;
  }
...
ORBIT_CLIENT_SDK_API int32_t OrbitRPCDispatcher::on_rpc_req_message(::atframework::orbit::OrbitRpcMessage& orbit_msg) {
  int32_t ret = 0;
  do {
    OrbitClientRuntime::me()->log(
    ...
    if (rpc_name.empty()) {
      ret = ::atframework::orbit::EN_ORBIT_ERROR_CODE_RPC_NAME_NOTFOUND;
      break;
    }
    if (task_action_map_by_name_.empty()) {
      ret = ::atframework::orbit::EN_ORBIT_ERROR_CODE_TASK_ACTION_EMPTY;
      break;
    }
    ...
    ret = ::atframework::orbit::EN_ORBIT_ERROR_CODE_TASK_ACTION_NOTFOUND;
  } while (false);
  on_create_task_failed(orbit_msg, ret);
  return ret;
}

ORBIT_CLIENT_SDK_API int32_t OrbitRPCDispatcher::on_rpc_rsp_message(::atframework::orbit::OrbitRpcMessage& orbit_msg) {
  OrbitClientRuntime::me()->log(
      OrbitClientLogLevel::kInfo, __FILE__, __LINE__,
...
ORBIT_CLIENT_SDK_API int32_t OrbitRPCDispatcher::send_rsp_to_proc(::atframework::orbit::OrbitRpcMessage& orbit_msg) {
...
ORBIT_CLIENT_SDK_API int OrbitRPCDispatcher::_register_action(const std::string& rpc_full_name,
  ...
  if (task_action_map_by_name_.end() != iter) {
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_ALREADY_REGISTER_ACTION;
  }
  task_action_map_by_name_[rpc_full_name] = action;
  return 0;
}

void OrbitRPCDispatcher::on_create_task_failed(::atframework::orbit::OrbitRpcMessage& orbit_msg, int32_t ret_code) {
  if (!orbit_msg.has_head()) {
    return;
  }
  ...
  ::atframework::orbit::OrbitRpcMessage rsp;
      ...
      if (callback != nullptr) {
        ...
        if (OrbitClientRuntime::me()->enabled_io_thread()) {
          OrbitClientRuntime::me()->post_to_caller_thread([callback]() {
            ::atframework::orbit::OrbitRpcMessage orbit_msg;
            orbit_msg.mutable_head()->set_error_code(::atframework::orbit::EN_ORBIT_ERROR_CODE_TIMEOUT);
            callback(orbit_msg);
          });
        } else {
          ::atframework::orbit::OrbitRpcMessage orbit_msg;
