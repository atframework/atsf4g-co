...
bool try_parse_uint64_argument(const std::string &input, uint64_t &output) {
  ...
  output = std::strtoull(input.c_str(), &end_ptr, 0);
...
struct atapp_log_sink_for_orbit {
  ...
  explicit atapp_log_sink_for_orbit(OrbitClientLogCallback log) : log_(log) {}
...
ORBIT_CLIENT_SDK_API bool OrbitClientRuntime::enabled_io_thread() const { return options_.io_thread; }

ORBIT_CLIENT_SDK_API void OrbitClientRuntime::post_to_io_thread(std::function<void()> task) {
  if (!task) {
    return;
  }
  io_task_.push(std::move(task));
}

ORBIT_CLIENT_SDK_API void OrbitClientRuntime::post_to_caller_thread(std::function<void()> task) {
  if (!task) {
    return;
  }

  caller_task_.push(std::move(task));
}
...
ORBIT_CLIENT_SDK_API int OrbitClientRuntime::init(uint64_t app_id, const OrbitClientOptions &options,
  ...
  if (state == OrbitClientRuntimeState::kConnecting || state == OrbitClientRuntimeState::kRunning ||
      state == OrbitClientRuntimeState::kStopping) {
  ...
  configured_ = true;
  ORBIT_LOG(OrbitClientLogLevel::kInfo,
            LOG_WRAPPER_FWAPI_FORMAT("runtime begin connecting, app_id={}", resolved_app_id));
...
bool OrbitClientRuntime::wait_connect() {
  ...
  OrbitClientRuntimeState state = state_.load();
  if (state != OrbitClientRuntimeState::kIdle && state != OrbitClientRuntimeState::kStopped) {
...
int32_t OrbitClientRuntime::notify_seed_process_ready_inner() {
  ...
  if (send_result < 0) {
    ORBIT_LOG(OrbitClientLogLevel::kError,
              LOG_WRAPPER_FWAPI_FORMAT("failed to send client_start request, code={}", send_result));
    return send_result;
  }
...
ORBIT_CLIENT_SDK_API int32_t OrbitClientRuntime::notify_seed_process_ready() {
  ...
  if (enabled_io_thread()) {
    post_to_io_thread([this] { notify_seed_process_ready_inner(); });
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_SUCCESS;
  }
  return notify_seed_process_ready_inner();
}

int32_t OrbitClientRuntime::blocking_seed_process() {
  while (true) {
    OrbitClientRuntimeState state = state_.load();
    ...
    if (res < 0) {
      ORBIT_LOG(OrbitClientLogLevel::kError, LOG_WRAPPER_FWAPI_FORMAT("process_fork_request failed, code={}", res));
      return res;
    }
    if (!is_seed_process()) {
      // 变成了Child进程 退出循环
...
int32_t OrbitClientRuntime::on_received_fork_request(const ::atframework::orbit::ATDForkSeedClientReq &request) {
  if (!is_seed_process()) {
...
void OrbitClientRuntime::on_received_stop_request(const ::atframework::orbit::ATDStopClientReq &request) {
...
int32_t OrbitClientRuntime::process_fork_request() {
  ...
  if (!pending_fork_requests_.try_pop(request)) {
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_SUCCESS;
  }

#if defined(__linux__) || defined(__unix__)
  // Fork
  // TODO(yousongyang) atapp 需要在fork前和后处理 现在先略过了
  pid_t child_pid = fork();
  if (child_pid < 0) {
    ORBIT_LOG(
        OrbitClientLogLevel::kError,
        LOG_WRAPPER_FWAPI_FORMAT("process_fork_request rejected: fork failed, errno={}", static_cast<int>(errno)));
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_PARAM_ERROR;
  }
  ...
  return init(request.app_id(), options_,
              callbacks_);  // TODO(yousongyang) Init内部应该还有问题 但是传入参数是正确的
}
...
ORBIT_CLIENT_SDK_API void OrbitClientRuntime::tick() {
  ...
  // 处理 caller_task
  {
    ...
    while (caller_task_.try_pop(task)) {
      if (!task) {
        continue;
      }
      task();
...
ORBIT_CLIENT_SDK_API int32_t OrbitClientRuntime::send_to_server(
    const std::string &payload, OrbitClientRpcCallback<::atframework::orbit::ATDSendToServerRsp> callback,
  ...
  if (send_result < 0) {
    ORBIT_LOG(OrbitClientLogLevel::kError,
              LOG_WRAPPER_FWAPI_FORMAT("failed to send send_to_server request, code={}", send_result));
    return send_result;
  }
...
ORBIT_CLIENT_SDK_API int32_t OrbitClientRuntime::request_end(::atframework::orbit::EnClientExitReason reason,
                                                             int32_t exit_code, const std::string &custom_data) {
  if (enabled_io_thread()) {
    post_to_io_thread([this, reason, exit_code, custom_data] { request_end_inner(reason, exit_code, custom_data); });
    return ::atframework::orbit::EN_ORBIT_ERROR_CODE_SUCCESS;
  }
  return request_end_inner(reason, exit_code, custom_data);
}
...
int OrbitClientRuntime::on_atapp_forward_response(::atframework::atapp::app &app,
  ...
  if (&app == app_.get() && source.id == agent_bus_id_ && error_code < 0) {
    ORBIT_LOG(OrbitClientLogLevel::kWarning,
              LOG_WRAPPER_FWAPI_FORMAT("send message to agent failed, code={}", error_code));
...
int OrbitClientRuntime::on_atapp_connected(::atframework::atapp::app &app, ::atbus::endpoint &ep, int status) {
  ...
  if (&app == app_.get() && 0 == status && 0 == agent_bus_id_) {
    ...
    ORBIT_LOG(OrbitClientLogLevel::kInfo, LOG_WRAPPER_FWAPI_FORMAT("agent connected, endpoint id={}", agent_bus_id_));
  }
  return 0;
}
...
int OrbitClientRuntime::on_atapp_disconnected(::atframework::atapp::app &app, ::atbus::endpoint &ep, int status) {
  ...
  if (&app == app_.get() && 0 != agent_bus_id_ && ep.get_id() == agent_bus_id_) {
    agent_bus_id_ = 0;
    ORBIT_LOG(OrbitClientLogLevel::kWarning, LOG_WRAPPER_FWAPI_FORMAT("agent disconnected, status={}", status));
  }
...
void OrbitClientRuntime::set_state(OrbitClientRuntimeState next_state) { state_.store(next_state); }
