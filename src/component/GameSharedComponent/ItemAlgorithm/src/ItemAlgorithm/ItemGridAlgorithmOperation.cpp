...
ITEM_ALGORITHM_API ItemGridReplaceCheckedRequest ItemGridAlgorithm::check_replace(
    const ::excel::excel_config_type_traits::shared_ptr<::excel::config_group_t>& config_group,
...
ITEM_ALGORITHM_API ItemGridOperationResult ItemGridAlgorithm::replace(ItemGridReplaceCheckedRequest& checked_request) {
  if (checked_request.result.error_code != PROJECT_NAMESPACE_ID::EN_SUCCESS) {
...
ITEM_ALGORITHM_API ItemGridOperationResult ItemGridAlgorithm::move(ItemGridMoveCheckedRequest& checked_request) {
  if (checked_request.result.error_code != PROJECT_NAMESPACE_ID::EN_SUCCESS) {
...
bool ItemGridAlgorithm::check_move_request(
  ...
  for (auto& sub_req : checked_request.request.move_sub_entrys) {
    ...
    sub_req.item_row = is_occupy_flag() ? position_cfg->row_size() : 0;
    sub_req.item_col = is_occupy_flag() ? position_cfg->column_size() : 0;
  ...
  for (auto& add_req : checked_request.request.move_add_entrys) {
    ...
    add_req.item_row = is_occupy_flag() ? position_cfg->row_size() : 0;
    add_req.item_col = is_occupy_flag() ? position_cfg->column_size() : 0;
...
ITEM_ALGORITHM_API ItemGridMoveCheckedRequest ItemGridAlgorithm::check_move(
    const ::excel::excel_config_type_traits::shared_ptr<::excel::config_group_t>& config_group,
...
ITEM_ALGORITHM_API void ItemGridAlgorithm::apply_entries(
    const ::excel::excel_config_type_traits::shared_ptr<::excel::config_group_t>& config_group,
