...
/// @brief 批次内占位推演: 检查 (x, y) 起的矩形是否已被本批次占掉, 没有占用就整块占上
///
/// 与 ItemFiniteGridContainer::is_occupancy_blocked 同一语义, 但直接作用在
/// ItemFiniteGridCheckScratch::reserved_occupancy 上 (批次位图由 scratch 持有, 不再单独复制)。
/// @return true 表示矩形里已有格子被占 (调用方按 EN_ERR_ITEM_POSITION_OCCUPIED 处理)
bool reserve_area(ItemGridOccupancy& occupancy, int32_t x, int32_t y, int32_t item_row_size, int32_t item_col_size) {
...
// ============================================================
// check_add / add 的批次检查与单条执行钩子
//
// 本模式的道具必须占格 (行列尺寸来自配置); 批次内用位图副本推演区域占用。
// 不占格道具不属于本模式 (is_item_valid 已挡住), 这里只做兜底拒绝。
//
// 批次流程在基类 (持有入参视图 / 初始化检查 / 通用校验 / GUID 唯一 / 类型数量上限),
// 基类完成通用校验后一次调用本模式的批次检查钩子, 这里只补位置与占用规则:
//   * 位置配置与堆叠上限 (accumulation_limit <= 0 视为无限堆叠);
//   * 目标锚点已有条目时, 只有同类且都无 GUID 才能堆叠, 堆叠后不能超过上限;
//   * 目标锚点在容器里没有条目时, 检查矩形是否越界、是否被容器或本批次前面的请求占掉;
//   * 目标锚点在本批次内已被前面的请求预定时, 按同样两条规则推演。
// 同批次的格子占用放在 on_check_add 创建的 ItemFiniteGridCheckScratch 里。
// ============================================================

ITEM_ALGORITHM_API ItemOperationResult
ItemFiniteGridContainer::on_check_add(const ItemAddCheckedRequest& checked_request) const {
  ItemFiniteGridCheckScratch scratch;
  scratch.reserved_occupancy = occupy_grid_flag_;
  ...
  checked_request.requests.foreach ([&](const PROJECT_NAMESPACE_ID::DItemInstance& request) -> bool {
    ...
    if (existing_entry) {
      ...
      if (guid != 0 || existing_basic.guid() != 0 || existing_basic.type_id() != type_id) {
        ...
        FWINSTLOGWARNING(logger(), "check_add failed[{}]: position ({},{}) occupied, error={} ({})", i, target_pos.x,
      ...
      if (pending_it != scratch.pending_new_slots.end()) {
        if (guid != 0 || pending_it->second.type_id != type_id) {
          int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_POSITION_OCCUPIED;
          FWINSTLOGWARNING(logger(), "check_add failed[{}]: position ({},{}) pending occupied, error={} ({})", i,
        ...
        if (item_row <= 0 || item_col <= 0) {
          ...
          FWINSTLOGWARNING(logger(),
                           ...
                           "error={} ({})",
                           i, item_row, item_col, type_id, error_code,
                           PROJECT_NAMESPACE_ID::EnErrorCode_Name(error_code));
          result = {error_code, type_id};
          return false;
        }
        ...
        if (!is_item_in_range(target_pos.x, target_pos.y, item_row, item_col)) {
          int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_POSITION_OUT_OF_RANGE;
          FWINSTLOGWARNING(logger(), "check_add failed[{}]: position ({},{}) out of range, error={} ({})", i,
                           target_pos.x, target_pos.y, error_code, PROJECT_NAMESPACE_ID::EnErrorCode_Name(error_code));
          result = {error_code, type_id};
          return false;
        }

        if (reserve_area(scratch.reserved_occupancy, target_pos.x, target_pos.y, item_row, item_col)) {
          int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_POSITION_OCCUPIED;
          FWINSTLOGWARNING(logger(),
                           "check_add failed[{}]: position ({},{}) occupied by area, "
                           "error={} ({})",
                           i, target_pos.x, target_pos.y, error_code,
                           PROJECT_NAMESPACE_ID::EnErrorCode_Name(error_code));
          result = {error_code, type_id};
          return false;
        }

        scratch.pending_new_slots[target_pos] = ItemFiniteGridCheckScratch::PendingNewSlot{type_id, add_count};
      }
    }
...
ITEM_ALGORITHM_API ItemOperationResult ItemFiniteGridContainer::on_add_one(
  ...
  if (item_type_config == nullptr) {
    ...
    // 本条没动任何数据, 按跳过处理, 返回成功让整批继续 (与旧实现的 continue 一致)
    return {PROJECT_NAMESPACE_ID::EN_SUCCESS, 0};
  }

  if (!item_type_config->need_occupy_the_grid) {
    // 有限格子容器只接受占格道具
    FWINSTLOGERROR(logger(), "add failed: ungrid item is not allowed in finite grid, type={} guid={}", type_id, guid);
    return {PROJECT_NAMESPACE_ID::EN_SUCCESS, 0};
  }
  ...
  if (existing) {
    ...
    FWINSTLOGDEBUG(logger(), "add stack grid type={} count={} at ({},{}) entry_id={} total={}", type_id, add_count,
  ...
  // 请求是只读视图, 先拷一份, 把规范化后的位置写进这份副本再落位
  ...
  if (!insert_entry_into_group(entry)) {
    FWINSTLOGERROR(logger(),
                   "[EntrySortKeyError] add failed to insert entry type={} count={} guid={} at ({},{}) entry_id={}",
  ...
  FWINSTLOGDEBUG(logger(), "add new grid type={} count={} guid={} at ({},{}) entry_id={}", type_id, add_count,
                 entry->item_instance().item_basic().guid(), target_pos.x, target_pos.y, entry->entry_id());
  return {PROJECT_NAMESPACE_ID::EN_SUCCESS, 0};
}

// ============================================================
// check_sub / sub 的批次检查与单条执行钩子
//
// 本模式的道具都占格, 因此扣减只有两种定位方式:
//   * 带 GUID 的道具按 GUID 定位 (数量固定为 1)
//   * 其余按格子位置定位
// 基类已做完通用校验与"按类型总数够扣"判断, 这里按上述两种定位方式补本模式的校验,
// 并在本批次钩子自己的状态中累计同一 GUID / 锚点已被要求扣减的数量。
// ============================================================

ITEM_ALGORITHM_API ItemOperationResult
ItemFiniteGridContainer::on_check_sub(const ItemSubCheckedRequest& checked_request) const {
  ItemFiniteGridCheckScratch scratch;
  ...
  checked_request.requests.foreach ([&](const PROJECT_NAMESPACE_ID::DItemBasic& request) -> bool {
    ...
    // 占格道具: 查表取位置配置
    auto position_cfg = get_item_position_cfg(checked_request.config_group, request);
    if (position_cfg == nullptr) {
      int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_FOUND;
    ...
    if (guid != 0) {
      ...
      if (scratch.pending_guid_count.count(guid) > 0) {
      ...
      scratch.pending_guid_count[guid] += sub_count;
    } else {
      // 按位置预扣减, 跟踪同一位置多次扣减
      ...
      if (!position_entry) {
        int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_FOUND;
        FWINSTLOGWARNING(logger(), "check_sub failed[{}]: position ({},{}) empty, error={} ({})", i, target_pos.x,
                         target_pos.y, error_code, PROJECT_NAMESPACE_ID::EnErrorCode_Name(error_code));
        result = {error_code, type_id};
        return false;
      }
      ...
      if (entry_basic.type_id() != type_id) {
        int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_FOUND;
        FWINSTLOGWARNING(logger(), "check_sub failed[{}]: position ({},{}) type mismatch, error={} ({})", i,
                         target_pos.x, target_pos.y, error_code, PROJECT_NAMESPACE_ID::EnErrorCode_Name(error_code));
        result = {error_code, type_id};
        return false;
      }
      ...
      int64_t already_sub = scratch.pending_position_count[target_pos];
      if (current - already_sub < sub_count) {
        int32_t error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_ENOUGH;
        FWINSTLOGWARNING(logger(),
                         "check_sub failed[{}]: not enough at ({},{}), current={} already_sub={} sub={}, "
                         "error={} ({})",
      ...
      scratch.pending_position_count[target_pos] = already_sub + sub_count;
    }
...
ITEM_ALGORITHM_API ItemOperationResult ItemFiniteGridContainer::on_sub_one(ItemSubCheckedRequest& checked_request,
  ...
  if (current_count <= sub_count) {
    ...
    FWINSTLOGDEBUG(logger(), "sub remove all type={} count={} guid={} entry_id={} at ({},{})", type_id, current_count,
                   guid, entry->entry_id(), entry_pos.x, entry_pos.y);
  } else {
    // 部分扣减
...
// check_has 的批次检查钩子
//
// 带 GUID 的道具按 GUID 对账, 其余道具按格子位置对账。
// 基类只按类型总量判断够不够, 覆盖不到"指定 GUID / 锚点上的条目是否存在、类型是否一致、
// 数量是否够", 所以这里按定位方式逐条补; 同批次内同一 GUID / 锚点的多次需求在本地状态中累计。
// ============================================================

ITEM_ALGORITHM_API ItemOperationResult ItemFiniteGridContainer::on_check_has(
  ...
  requests.foreach ([&](const PROJECT_NAMESPACE_ID::DItemBasic& request) -> bool {
    ...
    if (item_type_config != nullptr && item_type_config->need_guid) {
      ...
      if (!guid_entry || guid_entry->item_instance().item_basic().type_id() != request.type_id()) {
        result = {PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_FOUND, request.type_id()};
        return false;
      }

      int64_t required_count = scratch.pending_guid_count[request.guid()] + request.count();
      if (guid_entry->item_instance().item_basic().count() < required_count) {
        result = {PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_ENOUGH, request.type_id()};
        return false;
      }
      scratch.pending_guid_count[request.guid()] = required_count;
    } else {
      ...
      if (!position_entry || position_entry->item_instance().item_basic().type_id() != request.type_id()) {
        result = {PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_FOUND, request.type_id()};
        return false;
      }

      int64_t required_count = scratch.pending_position_count[position] + request.count();
      if (position_entry->item_instance().item_basic().count() < required_count) {
        result = {PROJECT_NAMESPACE_ID::EN_ERR_ITEM_NOT_ENOUGH, request.type_id()};
        return false;
      }
      scratch.pending_position_count[position] = required_count;
    }
...
// ============================================================
// move / check_move — 有限格子模式
//
// move 只在跟踪位置的模式里有意义: 先扣掉来源条目 (整体移除时释放格子),
// 再把道具放到目标位置 (目标已有同类无 GUID 条目时合入)。
// ============================================================

ITEM_ALGORITHM_API ItemOperationResult ItemFiniteGridContainer::on_move_sub_one(ItemMoveCheckedRequest& checked_request,
  ...
  if (sub_is_whole) {
    ...
    FWINSTLOGDEBUG(logger(), "move sub all type={} count={} guid={} entry_id={} from ({},{})", type_id, source_count,
                   guid, request.entry->entry_id(), request.position.x, request.position.y);
  } else {
    // 部分扣减
...
ITEM_ALGORITHM_API ItemOperationResult ItemFiniteGridContainer::on_move_add_one(ItemMoveCheckedRequest& checked_request,
  ...
  if (target_entry) {
    ...
    FWINSTLOGDEBUG(logger(), "move add merge type={} count={} at ({},{}) entry_id={} total={}", type_id,
  ...
  *new_instance.mutable_item_basic()->mutable_position() = request.goal_position;
  ...
  FWINSTLOGDEBUG(logger(), "move add new type={} count={} guid={} at ({},{}) entry_id={}", type_id, request.op_count,
                 new_entry->item_instance().item_basic().guid(), request.position.x, request.position.y,
                 new_entry->entry_id());
  return {};
}

bool ItemFiniteGridContainer::check_move_request(
  ...
  for (auto& sub_req : checked_request.request.move_sub_entrys) {
    ...
    sub_req.item_row = position_cfg->row_size();
    sub_req.item_col = position_cfg->column_size();
    type_count_delta[type_id] -= sub_req.op_count;
  }

  // ============================================================
  // 2. 检查 Add Entry 重复性, op_count, 填充 Helper, 范围, accumulation_limit, GUID
  // ============================================================
  ...
  for (auto& add_req : checked_request.request.move_add_entrys) {
    ...
    add_req.item_row = position_cfg->row_size();
    add_req.item_col = position_cfg->column_size();
    ...
    if (!is_item_in_range(add_req.position.x, add_req.position.y, add_req.item_row, add_req.item_col)) {
      ...
      FWINSTLOGWARNING(logger(), "check_move failed: move_add position ({},{}) out of range, error={} ({})",
...
ITEM_ALGORITHM_API bool ItemFiniteGridContainer::on_check_move(ItemMoveCheckedRequest& checked_request) const {
  // 被调用时机: 基类 check_move 完成初始化检查与空请求判断后调用一次 (本模式的移动语义都在这里)。
  auto& error_code = checked_request.result.error_code;
  auto& error_type_id = checked_request.result.failed_type_id;
  ...
  // ============================================================
  // Phase 1 (Sub): 在预留对象上释放所有整体 Sub 的条目格子
  // ============================================================
  item_placement_reservation_ptr_t placement_reservation = create_placement_reservation();
  ...
  for (const auto& op : checked_request.request.move_sub_entrys) {
    ...
    if (placement_reservation) {
      release_reserved_position(op.position.x, op.position.y, op.item_row, op.item_col, *placement_reservation);
    }
  }
  ...
  for (const auto& op : checked_request.request.move_add_entrys) {
    ...
    if (has_unmoved_existing) {
      ...
      if (target_basic.guid() != 0 || target_basic.type_id() != op.type_id) {
        ...
        error_type_id = op.type_id;
        FWINSTLOGWARNING(logger(), "check_move failed: move target ({},{}) occupied by type={} guid={}, error={} ({})",
      ...
      if (target_total > op.accumulation_limit) {
        ...
        error_type_id = op.type_id;
        FWINSTLOGWARNING(logger(),
                         "check_move failed: merge stack overflow at ({},{}), total={} limit={}, error={} ({})",
    ...
    if (pending_it != pending_new_anchors.end()) {
      if (pending_it->second.has_guid || pending_it->second.type_id != op.type_id) {
        ...
        error_type_id = op.type_id;
        FWINSTLOGWARNING(logger(), "check_move failed: pending anchor ({},{}) occupied, error={} ({})", op.position.x,
      ...
      if (total > pending_it->second.accumulation_limit) {
        ...
        error_type_id = op.type_id;
        FWINSTLOGWARNING(logger(),
                         "check_move failed: pending stack overflow at ({},{}), total={} limit={}, error={} ({})",
    ...
    // ---- 3. 全新位置 — 检查放置可行性 ----
    if (placement_reservation &&
        is_occupancy_blocked(op.position.x, op.position.y, op.item_row, op.item_col, *placement_reservation)) {
      error_code = PROJECT_NAMESPACE_ID::EN_ERR_ITEM_POSITION_OCCUPIED;
      error_type_id = op.type_id;
      FWINSTLOGWARNING(logger(), "check_move failed: area ({},{}) occupied by pending placement, error={} ({})",
...
// load 的单条钩子
//
// 初始化后按条目载入: 同锚点同类无 GUID 条目合并, 否则新建并要求位置合法且不碰撞。
// 基类已做完通用校验 (条目字段 / 容器归属 / 位置字段 / 类型存在 / GUID 唯一 / 数量上限),
// 这里只做落位; 载入的是已经存在的存档数据, 不做堆叠上限检查。
// ============================================================

ITEM_ALGORITHM_API bool ItemFiniteGridContainer::on_load_one(const excel_config_group_ptr_t& config_group,
  ...
  auto item_type_config = ItemAlgorithmTypeOption::GetItemType(type_id);
  if (item_type_config == nullptr || !item_type_config->need_occupy_the_grid) {
    FWINSTLOGWARNING(logger(), "load failed: ungrid item is not allowed in finite grid, type={}", type_id);
    return false;
  }
  ...
  if (existing) {
    ...
    if (guid != 0 || existing_basic.guid() != 0 || existing_basic.type_id() != type_id) {
      FWINSTLOGWARNING(logger(), "load failed: position ({},{}) occupied, error conflict type={} guid={}", target_pos.x,
    ...
    set_entry_count(existing, old_count + add_count);
    notify_entry_count_changed(existing, old_count, old_count + add_count, context);
    FWINSTLOGINFO(logger(), "load success: stack grid type={} count={} at ({},{}) entry_id={} total={}", type_id,
  ...
  int32_t item_col = position_cfg->column_size();
  if (item_row <= 0 || item_col <= 0) {
    FWINSTLOGWARNING(logger(), "load failed: invalid item size row={} col={} type={}", item_row, item_col, type_id);
    return false;
  }
  if (!is_item_in_range(target_pos.x, target_pos.y, item_row, item_col)) {
    FWINSTLOGWARNING(logger(), "load failed: position ({},{}) out of range type={}", target_pos.x, target_pos.y,
                     type_id);
    return false;
  }
  if (check_collision(target_pos.x, target_pos.y, item_row, item_col)) {
    FWINSTLOGWARNING(logger(), "load failed: position ({},{}) collision type={}", target_pos.x, target_pos.y, type_id);
    return false;
  }
  ...
  if (!insert_entry_into_group(entry)) {
    FWINSTLOGERROR(logger(),
                   "[EntrySortKeyError] load failed to insert entry type={} count={} guid={} at ({},{}) entry_id={}",
                   type_id, add_count, guid, target_pos.x, target_pos.y, entry->entry_id());
  ...
  add_entry_index(*position_cfg, entry);
  notify_entry_count_changed(entry, 0, add_count, context);
  FWINSTLOGINFO(logger(), "load success: new grid type={} count={} guid={} at ({},{}) entry_id={}", type_id, add_count,
                entry->item_instance().item_basic().guid(), target_pos.x, target_pos.y, entry->entry_id());
  return true;
}

// ============================================================
// apply_entries — 有限格子模式
//
// 客户端同步: 按 entry_id 删除 / 新增或更新条目。同步包是外部输入, 每条都要自己校验。
// ============================================================

ITEM_ALGORITHM_API void ItemFiniteGridContainer::on_apply_remove_one(const excel_config_group_ptr_t& config_group,
  ...
  auto position_cfg = get_item_position_cfg(config_group, entry->item_instance().item_basic());
  if (position_cfg != nullptr) {
    // 位置索引 / 占位位图 / GUID 索引都随条目一起摘掉
    remove_entry_index(*position_cfg, entry);
  } else {
    FWINSTLOGERROR(logger(), "apply remove failed: position cfg not found entry_id={} type={}", entry->entry_id(),
                   type_id);
  }
...
ITEM_ALGORITHM_API void ItemFiniteGridContainer::on_apply_update_one(
  ...
  // 容器归属) 校验并按 entry_id 查过一次条目之后调用一次。
  ...
  if (existing) {
    ...
    // 排序键由容器按位置语义算, 位置变了要重新排序: 先从分组摘出, 刷键后再插回
    if (position_changed) {
      ...
      // 位置没变时索引不会随 add_entry_index 一起刷新, 这里按新 GUID 重挂
      if (old_guid != 0) {
