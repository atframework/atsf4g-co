...
// ============================================================
// 寻位公共实现 — 支持 DItemBasic / DItemInstance 两种输入
// ============================================================
...
bool ItemGridAlgorithm::find_positions_inner(
  ...
  for (const auto& ignore : ignore_item) {
    ...
    if (ignore_cfg == nullptr) {
      ITEM_ALGORITHM_LOG_WARNING_FMT("find_positions_for_basics failed: unknown ignore item type={}",
                                     ignore.type_id());
      return false;
    }
    ...
    if (ignore_cfg->need_guid) {
      ...
      if (guid_it == guid_index_.end() ||
        ...
        ITEM_ALGORITHM_LOG_WARNING_FMT("find_positions_for_basics failed: ignore guid={} not found",
                                       ignore.guid());
        return false;
      }
      ...
      if (total < required) {
        ITEM_ALGORITHM_LOG_WARNING_FMT("find_positions_for_basics failed: ignore type={} not enough",
                                       ignore.type_id());
        return false;
      }
  ...
  for (const auto& item : items) {
    ...
    // ---- care_item_size 模式：三段策略 + on_check_add 校验 + 拆堆放置 ----
    ...
    // 策略 3：游标扫描背包寻找空闲格子 (支持拆堆, 一个格子放满后继续扫下一个)
    if (remaining > 0) {
      ...
      for (int32_t y = start_y; y <= row_size_ - item_rows && remaining > 0; ++y) {
        const int32_t x_begin = (y == start_y) ? start_x : 0;
        for (int32_t x = x_begin; x <= column_size_ - item_cols && remaining > 0; ++x) {
  ...
  ITEM_ALGORITHM_LOG_DEBUG_FMT("find_positions_for_basics pass, {} positions found, {} failed",
...
bool ItemGridAlgorithm::find_positions_for_basics(
  ...
  return find_positions_inner(config_group, basics, ignore_item, success_item, failed_item);
}

bool ItemGridAlgorithm::find_positions_for_instances(
    const ::excel::excel_config_type_traits::shared_ptr<::excel::config_group_t>& config_group,
  ...
  return find_positions_inner(config_group, items, ignore_item, success_item, failed_item);
}
