...
task_action_transfer_channel::task_action_transfer_channel(dispatcher_start_data_type&& param)
    : base_type(std::move(param)) {}

task_action_transfer_channel::~task_action_transfer_channel() {}

const char* task_action_transfer_channel::name() const { return "task_action_transfer_channel"; }

task_action_transfer_channel::result_type task_action_transfer_channel::operator()() {
  ...
  for (auto& channel_snapshot : *req_body.mutable_snapshot()) {
    ...
    if (0 != forward_server_id) {
      if (req_body.forward_ttl() > logic_config::me()->get_logic_cfg().router().transfer_max_ttl()) {
        FCTXLOGERROR(get_shared_context(), "mq channel {} forward ttl exceeded limit",
    ...
    if (!channel || !channel->load_snapshot(get_shared_context(), std::move(channel_snapshot))) {
      FCTXLOGERROR(get_shared_context(), "mq channel {} load snapshot failed", channel_key.channel_id());
  ...
  for (auto& forward_group : forward_by_server_id) {
    ...
    if (res < 0) {
      FCTXLOGERROR(get_shared_context(), "forward transfer {} mq channel(s) to server {:#x} failed, res: {}({})",
...
int task_action_transfer_channel::on_success() { return get_result(); }

int task_action_transfer_channel::on_failed() { return get_result(); }
