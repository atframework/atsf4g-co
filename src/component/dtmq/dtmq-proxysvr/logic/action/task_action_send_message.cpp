...
DTMQ_PROXY_SERVICE_API task_action_send_message::result_type task_action_send_message::operator()() {
  ...
  if (0 != forward_server_id) {
    ...
    if (req_body.forward_ttl() > dtmq_proxysvr_cfg.forward_request_max_ttl()) {
      FWLOGERROR("forward channel {} send message to {} failed, transfer ttl exceeded.",
    ...
    FWLOGDEBUG("channel {} forward message to {}.", req_body.channel_key().channel_id(), forward_server_id);
    req_body.set_forward_ttl(req_body.forward_ttl() + 1);
    TASK_ACTION_RETURN_CODE(RPC_AWAIT_CODE_RESULT(
        rpc::dtmq::send_message(get_shared_context(), forward_server_id, req_body, rsp_body, is_stream_rpc())));
  }
