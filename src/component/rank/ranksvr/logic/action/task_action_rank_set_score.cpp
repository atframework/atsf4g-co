#include "task_action_rank_set_score.h"
...
RANK_SERVICE_API task_action_rank_set_score::task_action_rank_set_score(dispatcher_start_data_type&& param)
    : base_type(COPP_MACRO_STD_MOVE(param)) {}

RANK_SERVICE_API task_action_rank_set_score::~task_action_rank_set_score() {}

RANK_SERVICE_API const char* task_action_rank_set_score::name() const { return "task_action_rank_set_score"; }

RANK_SERVICE_API task_action_rank_set_score::result_type task_action_rank_set_score::operator()() {
  ...
  ret = rank_ptr->update_score(req_body.data());

  if (rank_ptr->get_data_version() > old_version) {
    PROJECT_NAMESPACE_ID::DRankEventLog log;
    log.mutable_update_score()->CopyFrom(req_body.data());
    rank_ptr->broadcast_events(get_shared_context(), std::move(log));
  }
  FWLOGDEBUG(" rank({}:{}) set score user:{}.{} score:{} err:{}", req_body.rank_key().rank_type(),
             req_body.rank_key().rank_instance_id(), req_body.data().user_key().user_id(),
             req_body.data().user_key().zone_id(), req_body.data().score(), ret);

  rsp_body.set_result(ret);
  TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
}

RANK_SERVICE_API int task_action_rank_set_score::on_success() { return get_result(); }

RANK_SERVICE_API int task_action_rank_set_score::on_failed() { return get_result(); }
