...
RANK_SERVICE_API task_action_rank_clear::task_action_rank_clear(dispatcher_start_data_type&& param)
    : base_type(COPP_MACRO_STD_MOVE(param)) {}

RANK_SERVICE_API task_action_rank_clear::~task_action_rank_clear() {}

RANK_SERVICE_API const char* task_action_rank_clear::name() const { return "task_action_rank_clear"; }

RANK_SERVICE_API task_action_rank_clear::result_type task_action_rank_clear::operator()() {
  ...
  ret = rank_ptr->clear_rank();

  if (rank_ptr->get_data_version() > old_version) {
    PROJECT_NAMESPACE_ID::DRankEventLog log;
    log.mutable_rank_clear()->CopyFrom(req_body.data());
    rank_ptr->broadcast_events(get_shared_context(), std::move(log));
  }
  FWLOGDEBUG("rank:({}:{}) clear finish err:{}", req_body.rank_key().rank_type(),
             req_body.rank_key().rank_instance_id(), ret);
  rsp_body.set_result(ret);
  TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
}

RANK_SERVICE_API int task_action_rank_clear::on_success() { return get_result(); }

RANK_SERVICE_API int task_action_rank_clear::on_failed() { return get_result(); }
