...
rpc::result_code_type rank_mirror_manager::get_mirror_data_from_db(
  ...
  while (++cur_batch_index <= mirror_info.max_slice_count()) {
    keys.emplace_back(owner_->get_key().rank_type(), owner_->get_key().rank_instance_id(),
                      owner_->get_key().sub_rank_type(), owner_->get_key().sub_rank_instance_id(),
                      logic_config::me()->get_local_zone_id(), mirror_info.mirror_id(), cur_batch_index);
...
bool rank_mirror_manager::check_mirror_dump_finish(int64_t mirror_id) const {
  ...
  const auto& success_mirror_list = meta_data_.success_mirror();
  for (const auto& mirror : success_mirror_list) {
...
void rank_mirror_manager::check_and_remove_mirror() {
  static const int32_t max_mirror_count =
      logic_config::me()->get_server_instance_config<PROJECT_NAMESPACE_ID::config::ranksvr_cfg>().max_mirror_count();
