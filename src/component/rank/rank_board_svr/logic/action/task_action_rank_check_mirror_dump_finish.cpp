...
RANK_SERVICE_API task_action_rank_check_mirror_dump_finish::task_action_rank_check_mirror_dump_finish(
    dispatcher_start_data_type&& param)
    : base_type(COPP_MACRO_STD_MOVE(param)) {}

RANK_SERVICE_API task_action_rank_check_mirror_dump_finish::~task_action_rank_check_mirror_dump_finish() {}

RANK_SERVICE_API const char* task_action_rank_check_mirror_dump_finish::name() const {
  return "task_action_rank_check_mirror_dump_finish";
}
...
task_action_rank_check_mirror_dump_finish::operator()() {
  ...
  rsp_body.set_mirror_id(req_body.mirror_id());
  bool is_finish = rank_ptr->get_mirror_manager()->check_mirror_dump_finish(req_body.mirror_id());
  rsp_body.set_is_dump_finish(is_finish);

  FWLOGDEBUG("({}:{}:{}:{}) check mirror dump finish mirror_id:{} is_finish:{}", req_body.rank_key().rank_type(),
             req_body.rank_key().rank_instance_id(), req_body.rank_key().sub_rank_type(),
             req_body.rank_key().sub_rank_instance_id(), req_body.mirror_id(), is_finish ? "true" : "false");

  TASK_ACTION_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SUCCESS);
}

RANK_SERVICE_API int task_action_rank_check_mirror_dump_finish::on_success() { return get_result(); }

RANK_SERVICE_API int task_action_rank_check_mirror_dump_finish::on_failed() { return get_result(); }
