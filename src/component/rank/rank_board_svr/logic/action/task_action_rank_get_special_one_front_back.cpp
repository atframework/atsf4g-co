...
RANK_SERVICE_API task_action_rank_get_special_one_front_back::result_type
task_action_rank_get_special_one_front_back::operator()() {
  ...
  if (!rank || !rank->is_readable()) {
    FWLOGERROR("current node is not readable node rank:({}:{}:{}:{}) node:{}", req_body.rank_key().rank_type(),
               ...
               req_body.rank_key().sub_rank_instance_id(), logic_config::me()->get_local_server_id());
