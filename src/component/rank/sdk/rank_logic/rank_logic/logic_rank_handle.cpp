...
RANK_LOGIC_SDK_API logic_rank_handle_key::logic_rank_handle_key(const PROJECT_NAMESPACE_ID::config::ExcelRankRule& rule)
...
RANK_LOGIC_SDK_API logic_rank_handle_key::logic_rank_handle_key(uint32_t rank_type, uint32_t instance_id,
...
RANK_LOGIC_SDK_API size_t logic_rank_handle_key_hash::operator()(const logic_rank_handle_key& key) const noexcept {
...
RANK_LOGIC_SDK_API logic_rank_handle_decl::logic_rank_handle_decl()
    : last_rank_cache_cursor_(nullptr), last_rank_cache_total_count_(0) {}

RANK_LOGIC_SDK_API logic_rank_handle_decl::~logic_rank_handle_decl() {}

RANK_LOGIC_SDK_API void logic_rank_handle_decl::reset_cursor_front() noexcept {
  if (last_rank_cache_.empty()) {
    last_rank_cache_cursor_ = nullptr;
    return;
  }

  last_rank_cache_cursor_ = last_rank_cache_.data();
}

RANK_LOGIC_SDK_API void logic_rank_handle_decl::reset_cursor_back() noexcept {
  if (last_rank_cache_.empty()) {
    last_rank_cache_cursor_ = nullptr;
    return;
  }

  last_rank_cache_cursor_ = last_rank_cache_.data() + last_rank_cache_.size() - 1;
}

RANK_LOGIC_SDK_API bool logic_rank_handle_decl::next_cursor() noexcept {
  if (last_rank_cache_.empty()) {
    return false;
  }

  if (last_rank_cache_cursor_ == nullptr || last_rank_cache_cursor_ < last_rank_cache_.data()) {
    last_rank_cache_cursor_ = last_rank_cache_.data();
    return true;
  }
  size_t offset = static_cast<size_t>(last_rank_cache_cursor_ - last_rank_cache_.data());
...
RANK_LOGIC_SDK_API bool logic_rank_handle_decl::previous_cursor() noexcept {
  if (last_rank_cache_.empty()) {
    return false;
  }

  if (last_rank_cache_cursor_ == nullptr ||
      last_rank_cache_cursor_ > last_rank_cache_.data() + last_rank_cache_.size() - 1) {
    last_rank_cache_cursor_ = last_rank_cache_.data() + last_rank_cache_.size() - 1;
    return true;
  }
  if (last_rank_cache_cursor_ < last_rank_cache_.data()) {
    return false;
  }
  --last_rank_cache_cursor_;
  return true;
...
RANK_LOGIC_SDK_API bool logic_rank_handle_decl::valid_cursor() const noexcept {
  if (last_rank_cache_.empty()) {
    return false;
  }
  ...
  return last_rank_cache_cursor_ >= last_rank_cache_.data() &&
         last_rank_cache_cursor_ < last_rank_cache_.data() + last_rank_cache_.size();
}

RANK_LOGIC_SDK_API gsl::string_view logic_rank_handle_decl::get_current_open_id() const noexcept {
  if (!valid_cursor()) {
...
RANK_LOGIC_SDK_API uint32_t logic_rank_handle_decl::get_current_user_zone_id() const noexcept {
  if (!valid_cursor()) {
    return 0;
  }

  return last_rank_cache_cursor_->zone_id;
}

RANK_LOGIC_SDK_API uint32_t logic_rank_handle_decl::get_current_no() const noexcept {
  if (!valid_cursor()) {
    return 0;
  }
...
RANK_LOGIC_SDK_API uint32_t logic_rank_handle_decl::get_current_score() const noexcept {
  if (!valid_cursor()) {
    return 0;
  }
...
RANK_LOGIC_SDK_API int64_t logic_rank_handle_decl::get_current_timestamp() const noexcept {
  if (!valid_cursor()) {
    return 0;
  }
...
RANK_LOGIC_SDK_API uint32_t logic_rank_handle_decl::get_current_total_count() const noexcept {
  return last_rank_cache_total_count_;
}

RANK_LOGIC_SDK_API uint32_t logic_rank_handle_decl::get_current_count() const noexcept {
  return static_cast<uint32_t>(last_rank_cache_.size());
}

RANK_LOGIC_SDK_API const logic_rank_handle_data* logic_rank_handle_decl::get_current_cursor() const noexcept {
  if (!valid_cursor()) {
    return nullptr;
  }
...
RANK_LOGIC_SDK_API gsl::span<logic_rank_handle_data> logic_rank_handle_decl::get_current_span() noexcept {
  return gsl::span<logic_rank_handle_data>{last_rank_cache_};
}

RANK_LOGIC_SDK_API gsl::span<const logic_rank_handle_data> logic_rank_handle_decl::get_current_span() const noexcept {
...
RANK_LOGIC_SDK_API logic_rank_handle_self_impl::logic_rank_handle_self_impl(uint32_t world_id, uint32_t zone_id)
    : world_id_(world_id), zone_id_(zone_id) {}

RANK_LOGIC_SDK_API logic_rank_handle_self_impl::~logic_rank_handle_self_impl() {}

RANK_LOGIC_SDK_API uint32_t logic_rank_handle_self_impl::get_world_id() const noexcept { return world_id_; }

RANK_LOGIC_SDK_API uint32_t logic_rank_handle_self_impl::get_zone_id() const noexcept { return zone_id_; }

RANK_LOGIC_SDK_API gsl::span<const uint32_t> logic_rank_handle_self_impl::get_current_sort_fields() const noexcept {
...
RANK_LOGIC_SDK_API gsl::span<const uint64_t> logic_rank_handle_self_impl::get_current_ext_fields() const noexcept {
  const logic_rank_handle_data* cursor = get_current_cursor();
  if (nullptr == cursor) {
    return {};
  }
...
RANK_LOGIC_SDK_API bool logic_rank_handle_self_impl::is_service_available() const noexcept { return true; }

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::get_top_rank(
  ...
  if (image != nullptr && image->mirror_id() != 0) {
    ret = RPC_AWAIT_TYPE_RESULT(
        rpc::rank_board::get_top_from_mirror(ctx, *rank, zone_id_, start, count, image->mirror_id(), output));
  } else {
    ret = RPC_AWAIT_TYPE_RESULT(rpc::rank_board::get_top(ctx, *rank, start, count, output));
  }
  ...
  if (ret != 0) {
    ...
    for (uint32_t i = 0; i < sz; ++i) {
      logic_rank_handle_self_impl_dump_rank_data(last_rank_cache_.at(i),
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::upload_score(
  ...
  auto ret = RPC_AWAIT_TYPE_RESULT(rpc::rank_board::update_score(ctx, *user, *rank, score, *custom_data));
  if (ret != 0) {
    FWLOGERROR("upload_score {},{},{},{} for {} failed, res: {}({})", world_id_, zone_id_, key.get_rank_type(),
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::clear_all(
  ...
  auto ret = RPC_AWAIT_TYPE_RESULT(rpc::rank_board::clear_rank(ctx, *rank));
  if (ret != 0) {
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::clear_special_one(
  ...
  int32_t ret = RPC_AWAIT_TYPE_RESULT(rpc::rank_board::remove_one(ctx, *user_rank_key, *rank));
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::increase_score(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
    const rank_callback_private_data& callback_data, logic_rank_user_extend_span user_extend_data) {
  rpc::context::message_holder<PROJECT_NAMESPACE_ID::DRankUserKey> user_rank_key{ctx};
  fetch_current_rank_key(openid, zone_id_, *user_rank_key);
  ...
  rpc::context::message_holder<PROJECT_NAMESPACE_ID::DRankCustomData> custom_data{ctx};
  fetch_self_rank_custom_data(callback_data, user_extend_data, *custom_data);
  int32_t ret = RPC_AWAIT_CODE_RESULT(rpc::rank_board::modify_score(ctx, *user_rank_key, *rank, score, *custom_data));
  if (ret != 0) {
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::decrease_score(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
    const rank_callback_private_data& callback_data, logic_rank_user_extend_span user_extend_data) {
  rpc::context::message_holder<PROJECT_NAMESPACE_ID::DRankUserKey> user_rank_key{ctx};
  fetch_current_rank_key(openid, zone_id_, *user_rank_key);
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::get_special_one(
  ...
  auto ret = RPC_AWAIT_TYPE_RESULT(rpc::rank_board::get_special_one(ctx, *user_rank_key, *rank, *board_data));
  if (ret) {
    RPC_RETURN_TYPE(rank_ret_t(ret));
  }
...
RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_self_impl::get_special_score(
...
RANK_LOGIC_SDK_API bool logic_rank_handle_self_impl::is_current(
    const PROJECT_NAMESPACE_ID::config::ExcelRankRule& /*cfg*/, time_t /*now*/,
...
RANK_LOGIC_SDK_API logic_rank_handle_variant::logic_rank_handle_variant(const logic_rank_handle_variant& other)
...
RANK_LOGIC_SDK_API logic_rank_handle_variant& logic_rank_handle_variant::operator=(
    const logic_rank_handle_variant& other) {
  if (this == &other) {
    return *this;
  }
  destructor_delegate();
...
RANK_LOGIC_SDK_API logic_rank_handle_variant::logic_rank_handle_variant(logic_rank_handle_variant&& other)
    : variant_type_(other.variant_type_),
      enable_image_(other.enable_image_),
      sort_type_(other.sort_type_),
      delegate_(reinterpret_cast<logic_rank_handle_decl*>(object_data_)),
...
// NOLINTNEXTLINE(cppcoreguidelines-noexcept-move-operations, performance-noexcept-move-constructor)
RANK_LOGIC_SDK_API logic_rank_handle_variant& logic_rank_handle_variant::operator=(logic_rank_handle_variant&& other) {
  if (this == &other) {
    return *this;
  }
...
RANK_LOGIC_SDK_API logic_rank_handle_variant::~logic_rank_handle_variant() { destructor_delegate(); }
...
void logic_rank_handle_variant::init_delegate(uint32_t world_id, uint32_t zone_id) {
  switch (variant_type_) {
    case variant_type::KSelfRank: {
      delegate_ = gsl::not_null<logic_rank_handle_decl*>{new (static_cast<void*>(object_data_))
...
RANK_LOGIC_SDK_API uint32_t logic_rank_handle_variant::get_world_id() const noexcept {
  return delegate_->get_world_id();
}

RANK_LOGIC_SDK_API uint32_t logic_rank_handle_variant::get_zone_id() const noexcept { return delegate_->get_zone_id(); }

RANK_LOGIC_SDK_API gsl::span<const uint32_t> logic_rank_handle_variant::get_current_sort_fields() const noexcept {
  return delegate_->get_current_sort_fields();
}

RANK_LOGIC_SDK_API gsl::span<const uint64_t> logic_rank_handle_variant::get_current_ext_fields() const noexcept {
  return delegate_->get_current_ext_fields();
}

RANK_LOGIC_SDK_API bool logic_rank_handle_variant::is_service_available() const noexcept {
  return delegate_->is_service_available();
}

RANK_LOGIC_SDK_API bool logic_rank_handle_variant::is_current(const PROJECT_NAMESPACE_ID::config::ExcelRankRule& cfg,
                                                              time_t now,
                                                              const logic_rank_handle_key& key) const noexcept {
  return delegate_->is_current(cfg, now, key);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::get_top_rank(
    rpc::context& ctx, const logic_rank_handle_key& key, uint32_t start, uint32_t count,
    PROJECT_NAMESPACE_ID::DRankImageData* image) {
  return delegate_->get_top_rank(ctx, key, start, count, image);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::upload_score(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
    const rank_callback_private_data& callback_data, logic_rank_user_extend_span user_extend_data) {
  return delegate_->upload_score(ctx, key, openid, score, callback_data, user_extend_data);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::clear_all(
    rpc::context& ctx, const logic_rank_handle_key& key, PROJECT_NAMESPACE_ID::DRankImageData* image) {
  return delegate_->clear_all(ctx, key, image);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::clear_special_one(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid) {
  return delegate_->clear_special_one(ctx, key, openid);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::increase_score(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
    const rank_callback_private_data& callback_data, logic_rank_user_extend_span user_extend_data) {
  return delegate_->increase_score(ctx, key, openid, score, callback_data, user_extend_data);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::decrease_score(
    rpc::context& ctx, const logic_rank_handle_key& key, gsl::string_view openid, uint32_t score,
    const rank_callback_private_data& callback_data, logic_rank_user_extend_span user_extend_data) {
  return delegate_->decrease_score(ctx, key, openid, score, callback_data, user_extend_data);
}

RANK_LOGIC_SDK_API rpc::rpc_result<rank_ret_t> logic_rank_handle_variant::get_special_one(
    rpc::context& ctx, logic_rank_handle_data& output, const logic_rank_handle_key& key, gsl::string_view openid,
    uint32_t up_count, uint32_t down_count, PROJECT_NAMESPACE_ID::DRankImageData* image) {
  RPC_RETURN_TYPE(
...
RANK_LOGIC_SDK_API std::string rank_user_key_to_openid(uint32_t user_zone_id, uint64_t user_id,
                                                       PROJECT_NAMESPACE_ID::DRankInstanceKey instance_key) {  // NOLINT
  return atfw::util::log::format("{}:{}:{}:{}", user_zone_id, user_id, instance_key.instance_type(),
...
RANK_LOGIC_SDK_API std::tuple<uint32_t, uint64_t, int32_t, int64_t> rank_openid_to_user_key(gsl::string_view openid) {
  ...
  if (second_idx == openid.end()) {
    return {atfw::util::string::to_int<uint32_t>(openid.substr(0, static_cast<size_t>(colon_idx - openid.begin()))),
            atfw::util::string::to_int<uint64_t>(openid.substr(static_cast<size_t>(colon_idx - openid.begin() + 1))), 0,
            0};
  }
