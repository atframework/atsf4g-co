...
EXPLICIT_NODISCARD_ATTR RANK_BOARD_SDK_API rpc::result_code_type get_top(
  ...
  ret = RPC_AWAIT_CODE_RESULT(rpc::rank_board::rank_get_top(ctx, destination_server_id, *request_body, *response_body));
  if (ret != 0) {
    RPC_RETURN_CODE(ret);
  }
...
EXPLICIT_NODISCARD_ATTR RANK_BOARD_SDK_API rpc::result_code_type get_special_one_front_back(
  ...
  ret = RPC_AWAIT_CODE_RESULT(
      rpc::rank_board::rank_get_special_one_front_back(ctx, destination_server_id, *request_body, *response_body));
  if (ret != 0) {
    RPC_RETURN_CODE(ret);
  }
...
EXPLICIT_NODISCARD_ATTR RANK_BOARD_SDK_API rpc::result_code_type update_score(
  ...
  int32_t ret =
      RPC_AWAIT_CODE_RESULT(rpc::rank_board::rank_set_score(ctx, destination_server_id, *request_body, *response_body));
  if (ret != 0) {
    RPC_RETURN_CODE(ret);
  }
  RPC_RETURN_CODE(response_body->result());
