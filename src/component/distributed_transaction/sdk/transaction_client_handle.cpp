...
static bool is_retriable_decision_error(int32_t result) {
  return result == PROJECT_NAMESPACE_ID::err::EN_DB_OLD_VERSION ||
         result == PROJECT_NAMESPACE_ID::err::EN_SYS_RPC_CALL_NOT_READY;
}

static bool is_retriable_create_error(int32_t result) {
  return is_retriable_decision_error(result) || result == PROJECT_NAMESPACE_ID::err::EN_DB_KEY_EXISTS;
}

}  // namespace

DISTRIBUTED_TRANSACTION_SDK_API transaction_client_handle::storage_type::storage_type() : is_submitting_{false} {}

DISTRIBUTED_TRANSACTION_SDK_API transaction_client_handle::storage_type::~storage_type() = default;
...
DISTRIBUTED_TRANSACTION_SDK_API rpc::result_code_type transaction_client_handle::create_transaction(
  ...
  output->data.mutable_configure()->set_resolve_max_times(options.resolve_max_times);
  output->data.mutable_configure()->set_lock_retry_max_times(options.lock_retry_max_times);
  protobuf_copy_message(*output->data.mutable_configure()->mutable_resolve_retry_interval(),
                        protobuf_from_chrono_duration(options.resolve_retry_interval));
  protobuf_copy_message(*output->data.mutable_configure()->mutable_lock_wait_interval_min(),
                        protobuf_from_chrono_duration(options.lock_wait_interval_min));
  protobuf_copy_message(*output->data.mutable_configure()->mutable_lock_wait_interval_max(),
  ...
  rpc::result_code_type::value_type res = RPC_AWAIT_CODE_RESULT(rpc::transaction_api::initialize_new_transaction(
      child_ctx, output->data, options.timeout, options.replication_read_count, options.replication_total_count,
...
DISTRIBUTED_TRANSACTION_SDK_API rpc::result_code_type transaction_client_handle::submit_transaction(
    rpc::context& ctx, storage_ptr_type& input_reference,
    std::unordered_set<std::string>* output_prepared_participators,
    std::unordered_set<std::string>* output_failed_participators) {
  // Callbacks may replace the caller's pointer. Keep this transaction alive until this call returns
  // and never apply its remaining decisions or notifications to a replacement object.
  storage_ptr_type input = input_reference;
  if (!input) {
    RPC_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SYS_PARAM);
  }

  if (input->is_submitting_) {
    RPC_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_TRANSACTION_ALREADY_RUN);
  }
  input->is_submitting_ = true;
  auto release_submission = gsl::finally([input] { input->is_submitting_ = false; });
  ...
  if (!vtable_ || !vtable_->prepare_participator || !vtable_->reject_participator ||
      (!input->data.configure().force_commit() && !vtable_->commit_participator)) {
    FWLOGERROR("transaction {} submit but required vtable callbacks are not fully set",
               input->data.metadata().transaction_uuid());
    RPC_RETURN_CODE(PROJECT_NAMESPACE_ID::err::EN_SYS_PARAM);
  }
  ...
  // the transaction.
  auto expired_time = protobuf_to_system_clock(input->data.metadata().expire_timepoint());
  if (atfw::util::time::time_utility::now() >= expired_time) {
    FWLOGERROR("transaction {} submit but already expired", input->data.metadata().transaction_uuid());
  ...
  input->data.mutable_metadata()->set_status(atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_PREPARED);

  rpc::result_code_type::value_type ret = PROJECT_NAMESPACE_ID::err::EN_SUCCESS;
  // 创建事务,强制自动提交的事务不需要创建协调者事务对象
  // 协调者对相同内容的 create 幂等成功，CAS 冲突或创建/删除在途时做有限次重试，
  // 使晚到的 create 重放和落后的副本也有满足 R 的机会
  if (!input->data.configure().force_commit()) {
    for (int32_t left_retry_times = kCoordinatorRpcRetryTimes; left_retry_times-- > 0;) {
      if (atfw::util::time::time_utility::now() >= expired_time) {
        ret = PROJECT_NAMESPACE_ID::err::EN_SYS_TIMEOUT;
        break;
      }
      ret = RPC_AWAIT_CODE_RESULT(rpc::transaction_api::create_transaction(child_ctx, input->data));
      if (ret >= 0 || !is_retriable_create_error(ret)) {
        break;
      }
    }
    if (ret < 0) {
      input->data.mutable_metadata()->set_status(old_status);
  ...
  const uint64_t prepare_attempts = static_cast<uint64_t>(input->data.configure().lock_retry_max_times()) + 1;
  for (uint64_t retry_times = 0; retry_times < prepare_attempts; ++retry_times) {
    ...
    if (vtable_ && vtable_->prepare_participator) {
      for (const auto& participator : input->data.participators()) {
        ...
        if ((ret >= 0 || ret == PROJECT_NAMESPACE_ID::err::EN_TRANSACTION_RESOURCE_PREEMPTED) &&
          ...
          FWLOGWARNING("transaction {} prepare participator {} but resource preempted, we will retry soon later",
                       input->data.metadata().transaction_uuid(), participator.second.participator_key());
        ...
        if (ret < 0) {
          FWLOGERROR("transaction {} prepare participator {} failed, error code: {}({})",
                     input->data.metadata().transaction_uuid(), participator.second.participator_key(), ret,
    ...
    if (retry_later) {
      ...
      auto delay_min = protobuf_to_chrono_duration(input->data.configure().lock_wait_interval_min());
      auto delay_max = protobuf_to_chrono_duration(input->data.configure().lock_wait_interval_max());
      ...
      FWLOGERROR("transaction {} sleep and wait failed, error code: {}({})", input->data.metadata().transaction_uuid(),
  ...
  bool coordinator_decision_confirmed = input->data.configure().force_commit();
  rpc::context::message_holder<transaction_metadata> coordinator_metadata(child_ctx);

  if (input->data.configure().force_commit()) {
    input->data.mutable_metadata()->set_status(
        prepare_complete ? atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_COMMITED
                         : atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_REJECTED);
  } else if (prepare_complete) {
    // 协调者的最终状态写入是幂等的；版本冲突或创建/删除在途时有限次重试。
    for (int32_t left_retry_times = kCoordinatorRpcRetryTimes; left_retry_times-- > 0;) {
      // A conflict response can arrive after the last prepare's deadline check. Stop active commits
      // at the original deadline and use the coordinator query below to resolve any persisted result.
      if (atfw::util::time::time_utility::now() >= expired_time) {
        ret = PROJECT_NAMESPACE_ID::err::EN_SYS_TIMEOUT;
        break;
      }
      ...
      if (ret >= 0) {
        protobuf_copy_message(*input->data.mutable_metadata(), *coordinator_metadata);
      }
      if (!is_retriable_decision_error(ret)) {
        break;
      }
    }
  } else {
  ...
  if (!input->data.configure().force_commit() && ret >= 0 &&
      (input->data.metadata().status() == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_COMMITED ||
       input->data.metadata().status() == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_REJECTED)) {
  ...
  if (!input->data.configure().force_commit() && !coordinator_decision_confirmed) {
    const uint32_t resolve_attempts = std::max<uint32_t>(1, input->data.configure().resolve_max_times());
    for (uint32_t retry_times = 0; retry_times < resolve_attempts; ++retry_times) {
      rpc::context::message_holder<transaction_blob_storage> coordinator_storage(child_ctx);
      rpc::result_code_type::value_type query_result = RPC_AWAIT_CODE_RESULT(
          rpc::transaction_api::query_transaction(child_ctx, input->data.metadata(), *coordinator_storage));
      if (query_result >= 0) {
        rpc::transaction_api::merge_storage(input->data, *coordinator_storage);
        if (input->data.metadata().status() == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_COMMITED ||
            input->data.metadata().status() == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_REJECTED) {
          coordinator_decision_confirmed = true;
      ...
      if (retry_times + 1 < resolve_attempts) {
        rpc::result_code_type::value_type wait_result = RPC_AWAIT_CODE_RESULT(
            rpc::wait(child_ctx, protobuf_to_chrono_duration(input->data.configure().resolve_retry_interval())));
        if (wait_result < 0) {
          ret = wait_result;
          break;
        }
  ...
  const auto coordinator_status = input->data.metadata().status();
  if (coordinator_decision_confirmed &&
      coordinator_status == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_COMMITED) {
    ret = PROJECT_NAMESPACE_ID::err::EN_SUCCESS;
    FWLOGDEBUG("transaction {} commit success", input->data.metadata().transaction_uuid());
    ...
    FWLOGWARNING("transaction {} rejected by coordinator", input->data.metadata().transaction_uuid());
  } else {
    if (ret >= 0) {
    ...
    FWLOGERROR("transaction {} has no coordinator terminal decision, error code: {}({})",
               input->data.metadata().transaction_uuid(), ret, protobuf_mini_dumper_get_error_msg(ret));
  }
  ...
  const bool should_reject = coordinator_decision_confirmed &&
                             coordinator_status == atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_REJECTED;
  if ((should_commit || should_reject) && (!input->data.configure().force_commit() || should_reject) && vtable_) {
    const uint32_t notify_attempts = std::max<uint32_t>(1, input->data.configure().resolve_max_times());
    for (const auto& participator : input->data.participators()) {
      ...
      if (input->data.configure().force_commit() && should_reject && !is_prepared) {
      ...
      if (notify_result < 0) {
        ...
        FWLOGWARNING("transaction {} notify {} participator {} failed after {} attempts, error code: {}({})",
                     input->data.metadata().transaction_uuid(), should_commit ? "commit" : "reject", participator_key,
...
DISTRIBUTED_TRANSACTION_SDK_API int32_t transaction_client_handle::set_transaction_data(
  ...
  if (input->data.metadata().status() >= atfw::distributed_system::EN_DISTRIBUTED_TRANSACTION_STATUS_PREPARED) {
    return PROJECT_NAMESPACE_ID::err::EN_TRANSACTION_ALREADY_RUN;
  }

  if (false == input->data.mutable_transaction_data()->PackFrom(data)) {
    FWLOGERROR("Pack transaction data from {} failed, message: {}", protobuf_mini_dumper_get_readable(data),
               input->data.transaction_data().InitializationErrorString());
...
DISTRIBUTED_TRANSACTION_SDK_API int32_t transaction_client_handle::add_participator(rpc::context& ctx,
  ...
  auto iter = input->data.mutable_participators()->find(participator_key);
  if (iter != input->data.mutable_participators()->end()) {
  ...
  protobuf_move_message((*input->data.mutable_participators())[participator_key], std::move(participator));
