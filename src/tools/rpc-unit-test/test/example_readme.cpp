...
CASE_TEST(rpc_unit_test_readme, ss_mock_and_expectations) {
  ...
  auto task = test.run_task(
      "readme_ss", std::chrono::seconds{2}, [](rpc::context &ctx) -> rpc::result_code_type {
...
CASE_TEST(rpc_unit_test_readme, dns_mock) {
  ...
  auto task = test.run_task(
      "readme_dns", std::chrono::seconds{2}, [](rpc::context &ctx) -> rpc::result_code_type {
...
CASE_TEST(rpc_unit_test_readme, db_mock_handler_and_backend) {
  ...
  auto task = test.run_task(
      "readme_db", std::chrono::seconds{2}, [](rpc::context &ctx) -> rpc::result_code_type {
  ...
  CASE_EXPECT_EQ(0, static_cast<int>(test.db().calls("login_auth", atframework::testing::mock_db::op_type::kv_get_all)));
