...
#include "libsimulator_uv/simulator_base.h"
...
#include "libsimulator_uv/linenoise.h"
#include "libsimulator_uv/simulator_active.h"

static simulator_base *g_last_simulator = nullptr;
...
int simulator_base::init() {
  ...
  reg_req()["sleep"].bind(detail::on_sys_cmd_sleep(this),
                          "<NUMBER[s/ms/m/h/d/w]> sleep timeout, the unit can be s(second), ms(millisecond), "
...
int simulator_base::run(int argc, const char *argv[]) {
  ...
  args_mgr_->start(argc, argv, false, nullptr);
  if (is_closing_) {
    return 0;
  }
  ...
  uv_close((uv_handle_t *)&async_cmd_, nullptr);

  if (signals_.is_used) {
    signals_.is_used = false;

    uv_signal_stop(&signals_.sigint);
    uv_close((uv_handle_t *)&signals_.sigint, nullptr);

    uv_signal_stop(&signals_.sigquit);
    uv_close((uv_handle_t *)&signals_.sigquit, nullptr);
  ...
  if (tick_timer_.is_used) {
    ...
    tick_timer_.timer.data = nullptr;
    uv_timer_stop(&tick_timer_.timer);
    uv_close((uv_handle_t *)&tick_timer_.timer, nullptr);
  }

  sleep_timer_.timer.data = nullptr;
  ...
  uv_close((uv_handle_t *)&sleep_timer_.timer, nullptr);
...
void simulator_base::libuv_on_async_cmd(uv_async_t *handle) {
  ...
  if (!self->is_closing() && self->shell_cmd_manager_.read_file_ios.is_open() &&
      !self->shell_cmd_manager_.read_file_ios.eof()) {
    std::string cmd;
...
void simulator_base::linenoise_completion(const char *buf, linenoiseCompletions *lc) {
  ...
  for (std::string &cmd : res.complete) {
...
simulator_base::linenoise_helper_t &simulator_base::linenoise_build_complete(const char *line, bool complete,
                                                                             bool hint) {
...
void simulator_base::linenoise_thd_main(void *arg) {
  ...
  while (is_continue && nullptr != g_last_simulator && !g_last_simulator->is_closing()) {
    ...
    // lost TTY may produce a EIO, we should end the loop
    if (nullptr == cmd_c && errno == EIO || errno == ENOTTY) {
      ++io_error_counter;
    } else if (errno != 0 && nullptr == cmd_c) {
#ifdef _WIN32
    ...
    if (io_error_counter > 16) {
      g_last_simulator->stop();
    }

    // skip empty line
    if (nullptr != cmd_c && *cmd_c != '\0') {
      uv_mutex_lock(&g_last_simulator->async_cmd_lock_);
