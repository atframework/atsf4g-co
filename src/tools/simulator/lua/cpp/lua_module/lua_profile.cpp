...
static int lua_profile_hook_run_fn_gen_stats_table(lua_State *L, lua_profile_stack_data::stack_ptr_t root_ptr) {
  ...
  lua_pushinteger(L, static_cast<lua_Integer>(
                         std::chrono::duration_cast<std::chrono::microseconds>(root_ptr->call_full_duration).count()));
  lua_setfield(L, -2, "call_full_duration");

  lua_pushinteger(L, static_cast<lua_Integer>(
                         std::chrono::duration_cast<std::chrono::microseconds>(root_ptr->call_inner_duration).count()));
  lua_setfield(L, -2, "call_inner_duration");

  lua_pushinteger(L,
                  static_cast<lua_Integer>(
                      std::chrono::duration_cast<std::chrono::microseconds>(root_ptr->call_native_duration).count()));
...
lua_profile::lua_profile()
    : inited_(false), enabled_(true), enabled_native_code_(true), origin_hook_(nullptr), origin_mask_(0) {}
...
std::string lua_profile::dump_to(const std::string &file_path) {
  ...
  (*out) << "File Name, Line, Function Name, Call Times, Full Call Clock(Total), Full Call Clock(Avg.), Inner Call "
            "Clock(Total), "
...
void lua_profile::Hook_Fn(lua_State *L, lua_Debug *ar) {
  ...
  // if (!native_func && nullptr != ar->name && 0 == UTIL_STRFUNC_STRCASE_CMP("alloc_id", ar->name)) {
  //     int k = 0;
  // }
...
void lua_profile::exit_lua_func(const prof_key_t &key) {
  lua_profile_call_data *this_stack = nullptr;
  while (true) {
    // 刚启动时可能lua执行栈并不在顶层
