...
struct lua_profile_stack_data {
  using clock_t = lua_profile_stat_data::clock_t;
  using prof_key_t = std::pair<std::string, int>;
  using stack_ptr_t = std::shared_ptr<lua_profile_stack_data>;
  using map_t = std::unordered_map<prof_key_t, stack_ptr_t, lua_profile_pair_hash>;
...
struct lua_profile_call_data {
  using prof_key_t = lua_profile_stack_data::prof_key_t;
  using clock_t = lua_profile_stat_data::clock_t;
  using stats_ptr = std::shared_ptr<lua_profile_stat_data>;
...
class lua_profile : public atfw::util::design_pattern::singleton<lua_profile> {
 public:
  ...
  using clock_t = lua_profile_stat_data::clock_t;
  using map_t = std::unordered_map<prof_key_t, prof_ptr, lua_profile_pair_hash>;
