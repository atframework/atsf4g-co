...
namespace script {
    namespace lua {
        ...
        public:
            ...
            typedef typename lua_binding_userdata_info<proxy_type>::userdata_type     userdata_type;
            ...
            typedef std::function<int(lua_State *, value_type *)>                     member_proxy_method_t;
        ...
        public:
            lua_binding_class(const char *lua_name, const char *namespace_, lua_State *L)
                : lua_state_(L), lua_class_name_(lua_name), owner_ns_(namespace_, L), class_static_table_(0), class_member_table_(0), class_metatable_(0) {
                register_class();
            ...
            lua_binding_class(const char *lua_name, lua_binding_namespace &ns)
                : lua_state_(ns.get_lua_state()), lua_class_name_(lua_name), owner_ns_(ns), class_static_table_(0), class_member_table_(0),
                  class_metatable_(0) {
                register_class();
            ...
            self_type &add_method(const char *func_name, R (TClassIn::*fn)(TParam... param)) {
                // typedef R (TClassIn::*fn_t)(TParam...);
...
#if defined(LIBATFRAME_UTILS_ENABLE_RTTI) && LIBATFRAME_UTILS_ENABLE_RTTI
                    return detail::unwraper_member_fn<R, TClassIn, TParam...>::LuaCFunction(L, dynamic_cast<const TClassIn *>(pobj), fn);
#else
                    return detail::unwraper_member_fn<R, TClassIn, TParam...>::LuaCFunction(L, static_cast<const TClassIn *>(pobj), fn);
#endif
                };
                lua_pushcclosure(state, __member_method_unwrapper, 1);
                lua_settable(state, get_member_table());
            ...
            static const char *get_lua_metatable_name() { return lua_binding_metatable_info<value_type>::get_lua_metatable_name(); }

            self_type &set_new(lua_CFunction f, const std::string &method_name = "new") UTIL_CONFIG_NOEXCEPT {
                lua_State *state = get_lua_state();
                // new 方法
            ...
            self_type &set_to_string(lua_CFunction f) UTIL_CONFIG_NOEXCEPT {
                lua_State *state = get_lua_state();
                // __tostring方法
            ...
            self_type &set_gc(lua_CFunction f) UTIL_CONFIG_NOEXCEPT {
                lua_State *state = get_lua_state();
        ...
        private:
            ...
            void finish_class() UTIL_CONFIG_NOEXCEPT {
        ...
        private:
            ...
            static int __tostring(lua_State *L) {
                ...
                lua_pop(L, 1);
                if (real_ptr) {
                    ss << " @ " << convert_proxy_to_class<value_type, proxy_type, std::is_convertible<proxy_type *, value_type *>::value>::convert(*real_ptr);
                } else {
                    ss << " @ nil";
                }
            ...
            static int __member_method_unwrapper(lua_State *L) {
                ...
                do {
                    ...
                    // 检测继承
                    lua_getfield(L, -1, "__inherits"); // Stack +3: (class metatable, userdata metatable, userdata metatable["__inherits"])
                    if (lua_istable(L, -1)) {
                        bool check_passed = false;
                        for (lua_Integer i = 1;; ++i) {
                            lua_rawgeti(
                                L, -1,
                                i); // Stack +4: (class metatable, userdata metatable, userdata metatable["__inherits"], userdata metatable["__inherits"][i])
                    ...
                    if (nullptr == class_type_name) {
                        ...
                        lua_pushfstring(L, "%s(%s) is not base of userdata %s", class_name, class_type_name, userdata_type_name ? userdata_type_name : "");
                    }
                ...
                value_type *object_ptr =
                    convert_proxy_to_class<value_type, proxy_type, std::is_convertible<proxy_type *, value_type *>::value>::convert(*proxy_ptr);
                if (!object_ptr) {
            ...
            static int __vtables_method_lookup_symbol(lua_State *L, int key_index, int start_vtable_field_index) {
                ...
                for (;; ++vtable_field_index) {
                    lua_rawgeti(L, vtable_index, vtable_field_index); // Stack: +3(metatable, __vtables, __vtables[vtable_field_index])
                    if (lua_isnoneornil(L, -1)) {
                    ...
                    lua_gettable(L, -2);         // Stack: +4(metatable, __vtables, __vtables[vtable_field_index], __vtables[vtable_field_index][KEY])
                    if (!lua_isnoneornil(L, -1)) {
                        lua_replace(L, lua_gettop(L) - 3); // Stack: +3(__vtables[vtable_field_index][KEY], __vtables, __vtables[vtable_field_index])
                ...
                for (auto i = 1; i <= inherit_table_len; ++i) {
                    lua_rawgeti(L, inherit_table_index, i); // Stack: +4(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i])
                    if (!lua_isstring(L, -1)) {
                    ...
                    luaL_getmetatable(
                        L, inherit_class_name); // Stack: +5(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], parent_metatable)
                    if (lua_istable(L, -1)) {
                        lua_createtable(L, 0, 0); // Stack: +6(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], parent_metatable, {})
                        lua_pushvalue(
                            L,
                            -2); // Stack: +7(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], parent_metatable, {}, parent_metatable)
                        lua_setmetatable(L, -2); // Stack: +6(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], parent_metatable, {})

                        // Append __vtables[vtable_next_field_index] = {}(with metatable = parent_metatable)
                        lua_rawseti(
                            L, vtable_index,
                            vtable_next_field_index); // Stack: +5(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], parent_metatable)
                        ++vtable_next_field_index;

                        // Set metatable["__inherits"][i] = parent_metatable
                        lua_rawseti(L, inherit_table_index, i); // Stack: +4(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i])
                        lua_pop(L, 1);                          // Stack: +3(metatable, __vtables, metatable["__inherits"])
                    } else {
                        FWLOGERROR("lua try to lookup {}'s inherit class {} but metatable not found.", class_name, inherit_class_name);
                        fn::print_traceback(L, "");
                        lua_createtable(L, 0, 0); // Stack: +6(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], nil, {})
                        // Set metatable["__inherits"][i] = {}
                        lua_rawseti(L, inherit_table_index, i); // Stack: +5(metatable, __vtables, metatable["__inherits"], metatable["__inherits"][i], nil)
