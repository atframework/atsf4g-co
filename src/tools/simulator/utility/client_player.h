...
class client_player : public simulator_player_impl {
 ...
 public:
  ...
  libatgateway_v2_c_context mutable_proto_context(libuv_ptr_t net);
  void destroy_proto_context(libuv_ptr_t net);

  libuv_ptr_t find_network(libatgateway_v2_c_context ctx);
  using simulator_player_impl::find_network;

  void connect_done(libatgateway_v2_c_context ctx);
 ...
 private:
  std::map<uint32_t, libatgateway_v2_c_context> proto_handles_;
