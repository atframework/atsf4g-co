...
set(GENERATE_FOR_PB_REPO_DIR "${CMAKE_CURRENT_LIST_DIR}/generate-for-pb")
set(GENERATE_FOR_PB_IPC_PY "${GENERATE_FOR_PB_REPO_DIR}/generator_ipc.py")
set(GENERATE_FOR_PB_MAKO_PY "${GENERATE_FOR_PB_REPO_DIR}/mako-generator.py")
set(GENERATE_FOR_PB_JINJA2_PY "${GENERATE_FOR_PB_REPO_DIR}/jinja2-generator.py")
...
if(NOT EXISTS "${GENERATE_FOR_PB_MAKO_PY}")
  file(RELATIVE_PATH GENERATE_FOR_PB_SUBMODULE_PATH "${PROJECT_SOURCE_DIR}" "${GENERATE_FOR_PB_REPO_DIR}")
  maybe_populate_submodule(GENERATE_FOR_PB "${GENERATE_FOR_PB_SUBMODULE_PATH}" "${GENERATE_FOR_PB_REPO_DIR}")
  if(NOT EXISTS "${GENERATE_FOR_PB_IPC_PY}")
    message(FATAL_ERROR "git submodule update --init -- ${GENERATE_FOR_PB_SUBMODULE_PATH} failed")
  endif()
endif()

# On Windows, python.exe is a console-subsystem executable: every time CMake (configure or build) spawns it from a GUI
# process (IDE/cmake-gui/MSBuild), Windows allocates a new console window which flashes on screen. Prefer pythonw.exe
# (GUI subsystem, no console) for client-side invocations when it exists next to the selected interpreter, and retry
# with Python3_EXECUTABLE when pythonw.exe fails without producing any captured output so that diagnostics remain
# visible.
...
# Run a generate-for-pb client command. When the no-window interpreter fails without any captured output (e.g. the
# interpreter itself failed to start), retry once with the console interpreter to surface the error.
...
set(GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD "generate-for-pb-cleanup-server-prebuild")
set(GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_POSTBUILD "generate-for-pb-cleanup-server-postbuild")
...
function(generate_for_pb_setup_cleanup_server_target)
  if(TARGET ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD})
    return()
  endif()
...
")
  ...
  add_custom_target(
    ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD}
    COMMAND "${CMAKE_COMMAND}" -P "${GENERATE_FOR_PB_CLEANUP_SERVER_SCRIPT}"
    WORKING_DIRECTORY "${GENERATE_FOR_PB_WORK_DIR}"
    COMMENT "Cleanup old generate-for-pb server")
  set_property(TARGET ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD} PROPERTY FOLDER "${PROJECT_NAME}/codegen")

  add_custom_target(
    ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_POSTBUILD} ALL
    COMMAND "${CMAKE_COMMAND}" -P "${GENERATE_FOR_PB_CLEANUP_SERVER_SCRIPT}"
    WORKING_DIRECTORY "${GENERATE_FOR_PB_WORK_DIR}"
    COMMENT "Cleanup old generate-for-pb server")
  set_property(TARGET ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_POSTBUILD} PROPERTY FOLDER "${PROJECT_NAME}/codegen")

  execute_process(COMMAND "${CMAKE_COMMAND}" -P "${GENERATE_FOR_PB_CLEANUP_SERVER_SCRIPT}"
                  WORKING_DIRECTORY "${GENERATE_FOR_PB_WORK_DIR}")
endfunction()

function(generate_for_pb_register_common_dependency_for_target)
  foreach(TARGET_NAME ${ARGN})
    if(TARGET ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD})
      # 依赖清理
      add_dependencies(${TARGET_NAME} ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD})
    endif()
    if(TARGET ${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_POSTBUILD})
      # 依赖清理
      add_dependencies(${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_POSTBUILD} ${TARGET_NAME})
    endif()
  endforeach()
endfunction()
...
function(generate_for_pb_print_output_files RULE_FILE OUTPUT_VAR)
  ...
  generate_for_pb_execute_client(_generate_for_pb_print_result _generate_for_pb_print_stdout
                                 _generate_for_pb_print_stderr ${_generate_for_pb_run_print_output_files_options})
...
function(generate_for_pb_codegen_flow FLOW_NAME)
  ...
  # Bake the generation command into a wrapper script so the build step runs with the no-window interpreter and retries
  # with the console interpreter when it fails without producing any output.
...
")
  ...
  add_custom_command(
    ...
    WORKING_DIRECTORY "${GENERATE_FOR_PB_WORK_DIR}"
    DEPENDS "${GENERATE_FOR_PB_CLEANUP_SERVER_TARGET_PREBUILD}"
            "${_generate_for_pb_rule_file}"
            "${GENERATE_FOR_PB_MAKO_PY}"
            "${GENERATE_FOR_PB_IPC_PY}"
            "${_generate_for_pb_run_script}"
  ...
  generate_for_pb_register_common_dependency_for_target(${_generate_for_pb_target_name})
  foreach(PROTOCOL_TARGET IN LISTS GENERATE_FOR_PB_CODEGEN_FLOW_PROTOCOL_TARGET)
