...
function(project_service_declare_protocol TARGET_NAME PROTOCOL_DIR)
  ...
  if(project_service_declare_protocol_USE_SERVICE_PROTOCOL)
    foreach(USE_SERVICE_PROTOCOL ${project_service_declare_protocol_USE_SERVICE_PROTOCOL})
      unset(__RESOLVE_PROTO_ALIAS_TARGET)
      get_target_property(__RESOLVE_PROTO_ALIAS_TARGET "protocol::${USE_SERVICE_PROTOCOL}" ALIASED_TARGET)
      if(NOT __RESOLVE_PROTO_ALIAS_TARGET)
  ...
  if(COMMAND project_build_tools_patch_protobuf_targets)
    ...
    # project_service_force_optimize_sources(${__FINAL_GENERATED_SOURCE_FILES} ${__FINAL_GENERATED_HEADER_FILES})
  endif()
  ...
  target_include_directories(
    ${TARGET_FULL_NAME} PUBLIC "$<BUILD_INTERFACE:${project_service_declare_protocol_OUTPUT_DIR}>"
...
function(project_service_declare_instance TARGET_NAME SERVICE_ROOT_DIR)
  ...
  generate_for_pb_collect_output_from_flows("${project_service_declare_instance_GENERATED_FLOW_NAMES}"
                                            _PROJECT_SERVICE_GENERATED_OUTPUT_FILES)
  generate_for_pb_collect_filtered_output_files(
    "${SERVICE_ROOT_DIR}" "${_PROJECT_SERVICE_GENERATED_OUTPUT_FILES}" _PROJECT_SERVICE_GENERATED_SOURCE_FILES
  ...
  if(project_service_declare_instance_IMPL_TARGET_NAME)
    generate_for_pb_add_dependencies("${project_service_declare_instance_IMPL_TARGET_NAME}"
                                     "${project_service_declare_instance_GENERATED_FLOW_NAMES}")
    target_compile_options("${project_service_declare_instance_IMPL_TARGET_NAME}"
  ...
  if(PROJECT_COMMON_PRIVATE_INCLUDE_DIRECTORIES)
    ...
    if(project_service_declare_instance_IMPL_TARGET_NAME)
      target_include_directories("${project_service_declare_instance_IMPL_TARGET_NAME}"
...
")
  ...
  if(project_service_declare_instance_IMPL_TARGET_NAME)
    ...
    if(project_service_declare_instance_INCLUDE_DIR)
      target_include_directories("${project_service_declare_instance_IMPL_TARGET_NAME}"
                                 PUBLIC "$<BUILD_INTERFACE:${project_service_declare_instance_INCLUDE_DIR}>")
    endif()
    set(project_service_declare_instance_CODE_TARGET_NAME "${project_service_declare_instance_IMPL_TARGET_NAME}")
  else()
  ...
  if(project_service_declare_instance_PCH_FILES)
    ...
    project_pch_tool_set_precompile_headers(
      "${project_service_declare_instance_CODE_TARGET_NAME}"
      PCH_INIT_WEIGHT_RATIO
      ${__current_pch_weight}
      FOLDER
