...
unset(PROJECT_COMMON_PRIVATE_INCLUDE_DIRECTORIES)
...
if(MSVC)
  list(APPEND PROJECT_COMMON_PRIVATE_COMPILE_OPTIONS "/bigobj")
endif()
...
if(PROJECT_TOOL_ENABLE_SPLIT_DEBUG_SYMBOL_SUFFIX)
  if(CMAKE_CXX_COMPILER_ID MATCHES "GNU")
    ...
    if(NOT PROJECT_TOOL_OBJCOPY AND CMAKE_OBJCOPY)
      set(PROJECT_TOOL_OBJCOPY "${CMAKE_OBJCOPY}")
    endif()
  elseif(CMAKE_CXX_COMPILER_ID MATCHES "AppleClang|Clang")
    if(CMAKE_CXX_COMPILER MATCHES "-([0-9]+)")
      find_program(PROJECT_TOOL_OBJCOPY NAMES "llvm-objcopy-${CMAKE_MATCH_1}")
    endif()
    if(NOT PROJECT_TOOL_OBJCOPY)
  ...
  elseif(MSVC)
    if(NOT PROJECT_COMMON_PRIVATE_COMPILE_OPTIONS MATCHES "/Z7|/Zi|/ZI"
       AND NOT CMAKE_CXX_FLAGS MATCHES "/Z7|/Zi|/ZI"
       AND NOT CMAKE_MSVC_DEBUG_INFORMATION_FORMAT)
      # Visual Studio generator will produce LNK1103 when sharing PCH and using /Zi, so we use /Z7 instead.
      if(PROJECT_ENABLE_PRECOMPILE_HEADERS AND CMAKE_GENERATOR MATCHES "Visual Studio")
...
function(project_tool_set_target_runtime_output_directory OUTPUT_DIR TARGET_NAME)
  ...
  foreach(PROJECT_OUTPUT_CONFIG IN LISTS project_tool_set_target_runtime_output_directory_CONFIG_TYPES)
    ...
    set_property(TARGET ${TARGET_NAME} PROPERTY "RUNTIME_OUTPUT_DIRECTORY_${PROJECT_OUTPUT_CONFIG_UPPER}"
                                                "${OUTPUT_DIR}")
  endforeach()
  if(MSVC)
