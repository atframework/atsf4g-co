...
入口遇到歧义时在 stderr 提示 `--build-dir`，以退出码 2 结束；不会任意选用其中一份。
旧配置归属记录和私有设置可只读复用；新写入进入 `.mcp-data/`。
旧 `downloads/`、`upstream/`、`runtime/` 和 `npm-cache/` 中的制品仍按固定版本探测复用。
tgrep 需要补丁或重建时，先将旧源码及所选 Cargo 缓存复制到新目录，保留离线构建能力；
不复制旧构建输出、Cargo 配置、凭据或锁，也不修改旧源码和依赖缓存。
已完成的旧 embedding 模型异步复制到新目录后使用，模型状态与锁不写回旧目录。
迁移期间其他 Agent 可以接入、查询状态；搜索返回 `INDEX_NOT_READY`。
最后一个 Agent 断连会取消迁移，清理临时副本，并阻止随后启动后端。
...
API Key 隐藏输入同样遵循取消规则；终端关闭时的 raw mode 恢复失败不会覆盖原始取消异常，
清理换行的延迟写入错误也不会在 UI 关闭后成为未捕获异常。
...
- Node/Bun/Deno 均通过隐藏输入清理冒烟；两份工程的安装、切换与卸载 dry-run 保持配置内容和 mtime。
  日志位于执行工作区 `.mcp-data/tmp/issue-259/`。

### Issue #261：旧缓存复用与异步模型迁移（2026-09-23）

- 源仓库与 UE 副本各执行 356 项 JavaScript 回归：354 通过、2 项既有 POSIX 用例跳过；
  Python 各 9 项通过。覆盖旧制品布局、固定版本拒绝、新缓存优先、缓存歧义、异步准备失败零配置写入，
  以及模型复制中多 Agent 接入、最后断连取消和复制失败清理。
- 从隔离旧缓存真实执行 tgrep `cargo build --release --locked --offline`；旧源码与依赖文件的内容和
  mtime 均未改变。新制品通过 Windows native watcher 修改、新增、重命名、删除、热启动与离线补同步，
  在线事件约 60–63 ms，临时缓存中的源码探针未被索引。
- 两份工具均通过 Node/Bun/Deno 三 Agent 的三个真实后端共享、查询、刷新、断连清理与重连；
  Sirchmunk 验证旧模型异步导入、固定 embedding 离线加载和文件名搜索，未调用远程 LLM。
  模型导入函数也分别在三种运行时验证异步复制与临时文件过滤。
- WSL Debian / Node 20.19.2 的 39 项准备与配置回归全部通过，另有 2 项迁移生命周期用例通过；
  本轮未重跑 Linux 全套或真实后端。安装、切换、卸载 dry-run 保持两份工程的配置内容和 mtime。
- 本轮日志与隔离夹具位于执行工作区 `.mcp-data/tmp/issue-261/`，已由 Git/P4 忽略规则排除。
