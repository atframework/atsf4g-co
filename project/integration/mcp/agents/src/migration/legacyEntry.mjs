...
import { serverEntry, entryExtraArgs } from '../entries.mjs';

export function hasCustomOptions(entry, repoRoot, launch = {}) {
  return Object.keys(entry).some((key) => !['type', 'command', 'args', 'cwd', 'enabled'].includes(key))
    ...
    || entry.enabled === false || entryExtraArgs(entry, repoRoot, launch).length > 0;
}

/** Translate only verified stdio fields. Unknown cross-format options need a manual decision. */
export function migrateLegacyEntry({ entry, from, to, repoRoot, backend, relative, launch = {} }) {
  ...
  const base = serverEntry(to, repoRoot, backend, launch);
  // An omitted default must not conflict with an explicit destination option.
  delete base.enabled;
  const extras = entryExtraArgs(entry, repoRoot, launch);
