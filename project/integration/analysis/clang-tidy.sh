...
max_issues=''
report_directory=''
disable_msvc_precompiled_header=0

print_usage() {
  printf '%s\n' \
    'Usage: clang-tidy.sh --repository-root <path> --build-directory <path> --clang-tidy-executable <path>' \
    '       --max-issues <count> --report-directory <path> [--disable-msvc-precompiled-header]'
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --report-directory)
      ...
      shift 2
      ;;
    --disable-msvc-precompiled-header)
      disable_msvc_precompiled_header=1
      shift
      ;;
    *)
...
if [ -z "$repository_root" ] || [ -z "$build_directory" ] || [ -z "$clang_tidy_executable" ] ||
   [ -z "$max_issues" ] || [ -z "$report_directory" ]; then
  print_usage >&2
  exit 2
fi
case "$max_issues" in
...
cleanup() {
  rm -f "$staged_file_list" "$unstaged_file_list" "$changed_file_list" "$analysis_file_list" "$report_file" \
...
if ! LC_ALL=C sort -u "$staged_file_list" "$unstaged_file_list" >"$changed_file_list"; then
  printf '%s\n' 'clang-tidy: failed to merge the staged and unstaged file lists.' >&2
  exit 2
fi
...
if [ "$changed_file_count" -eq 0 ]; then
  printf '%s\n' 'clang-tidy: no staged or unstaged C/C++ files to check.'
  exit 0
fi
...
while IFS= read -r changed_file || [ -n "$changed_file" ]; do
  ...
  if [ "$disable_msvc_precompiled_header" -eq 1 ]; then
    MSYS2_ARG_CONV_EXCL='--extra-arg=/Y-;--extra-arg=/WX-' \
      "$clang_tidy_executable" --quiet --use-color=false "-p=$build_directory" '--extra-arg=-Wno-error' \
      '--extra-arg=/Y-' '--extra-arg=/WX-' '--extra-arg=-Wno-unused-command-line-argument' \
      "--line-filter=$line_filter" "$changed_file" >"$current_report_file" 2>&1
  else
    "$clang_tidy_executable" --quiet --use-color=false "-p=$build_directory" '--extra-arg=-Wno-error' \
      "--line-filter=$line_filter" "$changed_file" >"$current_report_file" 2>&1
  fi
...
if [ "$issue_count" -gt "$max_issues" ]; then
  printf 'clang-tidy report for %s staged or unstaged C/C++ file(s):\n' "$changed_file_count" >&2
  cat "$report_file" >&2
  printf 'clang-tidy: %s issue(s) exceed the configured maximum of %s.\n' "$issue_count" "$max_issues" >&2
  exit 1
fi

printf 'clang-tidy: %s issue(s) found in %s staged or unstaged C/C++ file(s); maximum allowed is %s.\n' \
