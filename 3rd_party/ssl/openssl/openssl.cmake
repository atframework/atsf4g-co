    ...
    if(TARGET OpenSSL::SSL OR TARGET OpenSSL::Crypto)
      if(TARGET OpenSSL::Crypto)
        ...
        if(TARGET Threads::Threads)
          project_build_tools_patch_imported_link_interface_libraries(OpenSSL::Crypto ADD_LIBRARIES Threads::Threads ${CMAKE_DL_LIBS})
      ...
      if(TARGET OpenSSL::SSL)
        ...
        if(TARGET Threads::Threads)
          project_build_tools_patch_imported_link_interface_libraries(OpenSSL::SSL ADD_LIBRARIES Threads::Threads ${CMAKE_DL_LIBS})
        elseif(CMAKE_DL_LIBS)
...
if(NOT 3RD_PARTY_CRYPT_LINK_NAME)
  set(3RD_PARTY_OPENSSL_DEFAULT_VERSION "1.1.1i")
  set(3RD_PARTY_OPENSSL_GITHUB_TAG "OpenSSL_1_1_1i")
  ...
  set(3RD_PARTY_OPENSSL_BUILD_OPTIONS
      ...
      "--release"
      # "--api=1.1.1" # libwebsockets and atframe_utils has warnings of using deprecated APIs, maybe it can be remove later "no-deprecated"
  ...
  if(NOT OPENSSL_FOUND)
    ...
    set(3RD_PARTY_OPENSSL_BUILD_DIR
        "${PROJECT_3RD_PARTY_PACKAGE_DIR}/openssl-${3RD_PARTY_OPENSSL_DEFAULT_VERSION}/build_jobs_${PROJECT_PREBUILT_PLATFORM_NAME}")
    ...
    if(NOT MSVC)
      ...
      file(APPEND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-config.sh"
           "export PATH=\"${3RD_PARTY_OPENSSL_BUILD_DIR}:\$PATH\"${PROJECT_THIRD_PARTY_BUILDTOOLS_BASH_EOL}")
      file(APPEND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.sh"
           "export PATH=\"${3RD_PARTY_OPENSSL_BUILD_DIR}:\$PATH\"${PROJECT_THIRD_PARTY_BUILDTOOLS_BASH_EOL}")
      ...
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-config.sh" "../config"
                                                   ${3RD_PARTY_OPENSSL_BUILD_OPTIONS})
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.sh" "make"
                                                   ${3RD_PARTY_OPENSSL_BUILD_MULTI_CORE})
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.sh" "make" "install_sw"
                                                   "install_ssldirs")
      ...
      execute_process(COMMAND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-config.sh" WORKING_DIRECTORY ${3RD_PARTY_OPENSSL_BUILD_DIR})

      execute_process(COMMAND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.sh" WORKING_DIRECTORY ${3RD_PARTY_OPENSSL_BUILD_DIR})
      ...
      list(APPEND 3RD_PARTY_OPENSSL_BUILD_OPTIONS "no-makedepend" "-utf-8" "no-capieng" # "enable-capieng"  # 有些第三方库没有加入对这个模块检测的支持，比如
                                                                                        # libwebsockets
      )
      ...
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-config.bat" perl "../Configure"
                                                   ${3RD_PARTY_OPENSSL_BUILD_OPTIONS})
      file(APPEND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.bat"
           "set CL=${3RD_PARTY_OPENSSL_BUILD_MULTI_CORE}${PROJECT_THIRD_PARTY_BUILDTOOLS_BASH_EOL}")
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.bat" "nmake" "build_all_generated")
      project_expand_list_for_command_line_to_file("${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.bat" "nmake" "PERL=no-perl")
      ...
      execute_process(COMMAND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-config.bat" WORKING_DIRECTORY ${3RD_PARTY_OPENSSL_BUILD_DIR})

      execute_process(COMMAND "${3RD_PARTY_OPENSSL_BUILD_DIR}/run-build-release.bat" WORKING_DIRECTORY ${3RD_PARTY_OPENSSL_BUILD_DIR})
    endif()
