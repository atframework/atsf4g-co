...
6. For scope or level changes, preserve the three-layer model:
   ...
   `DMatchingScope` must never contain the selected level. Validate the client selection against server-owned
   level configuration before building the normalized candidate set. `DLevelSelect.level_ids` is a required, non-empty,
   unordered CS multi-select input. Lobby normalizes it before forwarding, and the room deterministically selects from the
   current intersection. Every candidate must resolve to the same requested `level_type` and server-owned matching pool.
7. For rule or faction changes, test boundary values, impossible assignments, multiple parties, and rollback after a
   ...
   migration atoms from newer donors. Traverse donors and their migration atoms in stable order; evaluate each atom once
   against the current source and target state, then immediately commit an accepted atom with the assignments produced by
   that evaluation. Do not build or sort a separate candidate-plan frontier. Re-evaluate both rooms after every commit.
   Bound both one target's migrations and total
   ...
   During matching and confirmation, store faction membership, fixed capacity, and a derived assigned-user-count cache.
   Treat membership as authoritative: rebuild and validate the cached count at room mutation boundaries, then use the
   cache in hot placement and rebalance scans instead of repeatedly resolving every Unit. Immediately before battle creation,
...
8. Keep room state and subscriber state separate:
   ...
   - CS responses and pushes use the smaller `DMatchingClientView`. It contains the stable `unit_id`, lifecycle/result,
     expiry, selected level, and finalized `faction_id`; it never contains
   ...
   - Matching does not maintain a separate client-view revision. Lobby alone advances and persists the Matchsvr WAL
     cursor; client message ordering uses the common CS message sequence.
