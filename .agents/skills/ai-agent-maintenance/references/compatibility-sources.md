...
| Codex / ChatGPT | [Build skills](https://developers.openai.com/codex/skills) | Initial context contains Skill name, description, and path within a bounded budget; Codex discovers repository `.agents/skills/` and loads the body only after selection. |
...
| Claude Code | [project memory](https://code.claude.com/docs/en/memory), [Skills](https://code.claude.com/docs/en/skills) | Claude reads `CLAUDE.md`; native project Skills live under `.claude/skills/`. This repository imports the compact `.agents/skills/README.md` as a fallback router instead of duplicating Skill bodies. |
