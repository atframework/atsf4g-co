...
- 需要为伪代码同步补测试用例伪代码。
- 需要根据 `src\component\orbit` 下的正式 `.proto` 更新 orbit 协议说明。
...
- `src\component\orbit` 下的 `.proto` 是最终事实源，文档与伪代码只能解释、归纳、映射这些协议，不能自行扩写未定义能力。
...
2. 如果是协议说明类更新，先从对应 `.proto` 提炼消息、字段和 RPC 方向，再同步到 `doc\orbit`。
...
6. 将内容写入最接近责任归属的 `doc\orbit\pseudocode` 子目录，按 `<module>.pseudo.h`、`<module>.pseudo.cpp`、`<module>_test.pseudo.cpp` 组织。
...
- 协议说明必须直接对应 `src\component\orbit` 下的正式 `.proto`，避免把旧设计草案写回文档。
