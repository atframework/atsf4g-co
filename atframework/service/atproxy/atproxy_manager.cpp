...
namespace atframe {
    namespace proxy {
        ...
        int atproxy_manager::init() {
            ...
            int ret =
                etcd_mod->add_watcher_by_type_name(get_app()->get_type_name(), std::bind(&atproxy_manager::on_watcher_notify, this, std::placeholders::_1));
            ...
            do {
                ...
                if (get_app()->get_bus_node()) {
                    ...
                    if (res >= 0) {
                        ...
                        FWLOGERROR("try to connect to proxy: {:#x}, address: {} failed, res: {}", iter->second.etcd_node.node_discovery.id(), *select_address,
                                   res);
                    }
