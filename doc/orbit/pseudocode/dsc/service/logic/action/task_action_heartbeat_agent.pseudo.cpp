...
static rpc_result_t write_heartbeat_agent_response(rpc_context_t& rpc_context) {
  (void)rpc_context;
  return 0;
}
...
rpc_result_t run_task_action_heartbeat_agent(runtime_handle_t& runtime,
  ...
  (void)runtime;
  if (!request.has_agent() || 0 == request.agent_id() || request.now_ms() <= 0) {
  ...
  auto registry_result = agent_registry.refresh_agent_heartbeat(request.agent_id(), request.now_ms(), load);
  if (registry_result != 0) {
    return build_rpc_error(registry_result);
  }

  return write_heartbeat_agent_response(rpc_context);
}
