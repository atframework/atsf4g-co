...
路由系统解决"有状态对象在哪个服务实例上"的问题：对象（如玩家、队伍）按 key 路由到归属实例，并在内存中
缓存、定时回存。
...
| `router_player_cache` / `router_player_manager` | `router/router_player_*` | 玩家路由特化（`player_cache` 对象、`pull_online_server`） |
