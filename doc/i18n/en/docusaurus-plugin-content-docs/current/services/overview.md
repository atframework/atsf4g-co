...
| `lobbysvr` | Lobby service (core logic service): login, kick-offline, player info, player async jobs (`async_jobs/`), global/user cache (`cache/`), rank board queries (`rank/`), dtmq channel event receiving (`logic/dtmq/`), ships with `sdk/` |
