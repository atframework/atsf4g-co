...
if [[ 0 -eq $? ]] && [[ $PROC_PID -gt 0 ]]; then
  NoticeMsg "tgf_TGF_prestop_succeed_TGF_tgf, {{ .Values.proc_name }} - {{ $bus_addr }} - pid $PROC_PID already stopped." | tee $LAST_RUNCMD_LOGFILE
  exit 0
fi
...
if [[ $PROC_PID -le 0 ]] || [[ 0 -eq $(CheckPidAndExePath "$SCRIPT_DIR/{{ .Values.proc_name }}" $PROC_PID) ]] ; then
    ...
    if [[ ${PIPESTATUS[0]} -ne 0 ]]; then
        ErrorMsg "tgf_TGF_prestop_failed_TGF_tgf, send prestop command to {{ .Values.proc_name }} - {{ $bus_addr }} failed." | tee -a $LAST_RUNCMD_LOGFILE
        exit 1
    fi

    status=$(cat $LAST_RUNCMD_LOGFILE | grep "Run Prestop Command Failed" | wc -l)
    if [[ "0" -ne "${status}" ]]; then
        ErrorMsg "tgf_TGF_prestop_failed_TGF_tgf, {{ .Values.proc_name }} - {{ $bus_addr }} run prestop failed." | tee -a $LAST_RUNCMD_LOGFILE
        exit 1
    fi
    ...
    if echo "$custom_cmd" | grep -q "server prestop success"; then
        NoticeMsg "tgf_TGF_prestop_succeed_TGF_tgf, prestop {{ .Values.proc_name }} - {{ $bus_addr }} done." | tee -a $LAST_RUNCMD_LOGFILE
        ...
        NoticeMsg "tgf_TGF_prestop_try_again_TGF_tgf, prestopping {{ .Values.proc_name }} - {{ $bus_addr }}, wait for a while." | tee -a $LAST_RUNCMD_LOGFILE
    else
        ErrorMsg "tgf_TGF_prestop_failed_TGF_tgf, prestop {{ .Values.proc_name }} - {{ $bus_addr }} return unknown, try again later." | tee -a $LAST_RUNCMD_LOGFILE
        exit 2
    fi
else
    NoticeMsg "tgf_TGF_prestop_succeed_TGF_tgf, send prestop command to {{ .Values.proc_name }} - {{ $bus_addr }} failed, not running." | tee $LAST_RUNCMD_LOGFILE
fi
{{- end }}
