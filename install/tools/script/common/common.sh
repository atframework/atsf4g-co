...
function CheckProcessRunning() {
  ...
  if [[ -n "${MSYSTEM}" ]]; then
      # MSYS 环境：使用 ps -W 列出所有 Windows 进程，再用 grep 匹配
      if ps -W | awk '{print $4}' | grep -q "^${PROC_PID}$"; then
          return 1
      fi
  else
      # 纯 Linux / Cygwin 环境：标准的 ps -p 检查
...
function CheckWindowsPidAndExePath() {
  # parameters: <checked executable file path> <pid>
  if [[ $# -lt 2 ]]; then
    echo "1"
    return 1
  ...
  ACTUAL_EXE_PATH=$(ps -W | awk -v pid="$PROC_PID" '$4 == pid {print $8}')

  # 2. 判断：PID 存在（路径不为空）且 路径与预期一致
  if [[ -n "$ACTUAL_EXE_PATH" ]] && [[ "$EXPECT_EXE_PATH" == "$ACTUAL_EXE_PATH" ]]; then
      echo "0"
      return 0
  fi
...
function CheckPidAndExePath() {
  if [[ -n "${MSYSTEM}" ]]; then
    CheckWindowsPidAndExePath "$@"
    return $?
  fi

  CheckLinuxPidAndExePath "$@"
  return $?
}