...
static client_libuv_data_t g_client;

struct proto_wrapper {
  libatgateway_v2_c_context ctx;
  proto_wrapper() : ctx(libatgateway_v2_c_create()) {}
  ~proto_wrapper() {
    if (nullptr != ctx) {
      libatgateway_v2_c_destroy(ctx);
    }
  }
};
...
struct client_session_data_t {
  ...
  size_t sec_send_bound = 1;
  size_t sec_recv_count = 0;
  size_t sum_recv_count = 0;
  size_t sec_send_count = 0;
  size_t sum_send_count = 0;
  size_t sec_recv_size = 0;
  size_t sum_recv_size = 0;
  size_t sec_send_size = 0;
  size_t sum_send_size = 0;
};

// NOLINTNEXTLINE(cppcoreguidelines-avoid-non-const-global-variables)
static client_session_data_t g_client_sess;
...
#if _MSC_VER
  uint64_t len = 0;
  libatgateway_v2_c_read_alloc(g_client_sess.proto->ctx, suggested_size, &buf->base, &len);
...
static void libuv_tcp_recv_read_fn(uv_stream_t *, ssize_t nread, const uv_buf_t *buf) {
  ...
  if (g_client_sess.proto) {
    ...
    int32_t errcode = 0;
    libatgateway_v2_c_read(sess_proto->ctx, static_cast<int32_t>(nread), buf->base, static_cast<size_t>(nread),
                           &errcode);
    if (0 != errcode) {
...
static void libuv_tcp_connect_callback(uv_connect_t *req, int status) {
  ...
  if (0 != ret) {
    ...
    libatgateway_v2_c_close(sess_proto->ctx, 0);
  }
}
...
static void libuv_dns_callback(uv_getaddrinfo_t *req, int status, struct addrinfo *res) {
  ...
  do {
    ...
    if (AF_INET == res->ai_family) {
      ...
      uv_ip4_addr(ip, g_port, reinterpret_cast<sockaddr_in *>(&real_addr));
    } else if (AF_INET6 == res->ai_family) {
      sockaddr_in6 *res_c = reinterpret_cast<sockaddr_in6 *>(res->ai_addr);
      char ip[40] = {0};
      uv_ip6_name(res_c, ip, sizeof(ip));
      uv_ip6_addr(ip, g_port, reinterpret_cast<sockaddr_in6 *>(&real_addr));
    } else {
      fprintf(stderr, "uv_tcp_connect failed, ai_family not supported: %d\n", res->ai_family);
      break;
    }

    int res_code = uv_tcp_connect(&g_client.tcp_req, &g_client.tcp_sock, reinterpret_cast<sockaddr *>(&real_addr),
...
static int send_next_hello_message() {
  ...
  int ret = libatgateway_v2_c_post_msg(g_client_sess.proto->ctx, msg, msg_sz);
...
static int proto_inner_callback_on_message(libatgateway_v2_c_context, const void *buffer, uint64_t sz) {
  if (g_client_sess.print_msg && nullptr != buffer && sz > 0) {
    printf("[recv message]: %s\n",
           std::string(reinterpret_cast<const char *>(buffer), static_cast<size_t>(sz)).c_str());
  }
...
static int proto_inner_callback_on_new_session(libatgateway_v2_c_context, uint64_t *sess_id) {
  printf("create session 0x%llx\n", nullptr == sess_id ? 0 : static_cast<unsigned long long>(*sess_id));
  return 0;
}

// useless
static int proto_inner_callback_on_reconnect(libatgateway_v2_c_context, uint64_t sess_id) {
...
static int proto_inner_callback_on_handshake(libatgateway_v2_c_context ctx, int32_t status) {
  char buffer[4096];
  if (0 == status) {
    libatgateway_v2_c_get_info(ctx, buffer, 4096);
    printf("[Info]: handshake done\n%s\n", buffer);
    g_client_sess.session_id = libatgateway_v2_c_get_session_id(ctx);
  } else {
...
static int proto_inner_callback_on_handshake_update(libatgateway_v2_c_context ctx, int32_t status) {
  char buffer[4096];
  if (0 == status) {
    libatgateway_v2_c_get_info(ctx, buffer, 4096);
...
static int proto_inner_callback_on_error(libatgateway_v2_c_context, const char *filename, int line, int errcode,
                                         const char *errmsg) {
...
static void libuv_tick_timer_callback(uv_timer_t *handle) {
  ...
  if (libatgateway_v2_c_is_closing(g_client_sess.proto->ctx)) {
    return;
  }

  if (!libatgateway_v2_c_is_handshake_done(g_client_sess.proto->ctx)) {
    return;
  }
...
int main(int argc, char *argv[]) {
  ...
  libatgateway_v2_c_global_cleanup_algorithms();
  return ret;
}
