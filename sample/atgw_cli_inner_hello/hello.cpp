...
static void libuv_tcp_recv_read_fn(uv_stream_t *, ssize_t nread, const uv_buf_t *buf) {
  ...
  if (g_client_sess.proto) {
    ...
    int32_t errcode = 0;
    libatgw_inner_v1_c_read(sess_proto->ctx, static_cast<int32_t>(nread), buf->base, static_cast<size_t>(nread),
                            &errcode);
    if (0 != errcode) {
...
static void libuv_tcp_connect_callback(uv_connect_t *req, int status) {
  ...
  if (g_client_sess.proto && g_client_sess.allow_reconnect) {
    ...
    ret = libatgw_inner_v1_c_reconnect_session(sess_proto->ctx, g_client_sess.session_id, crypt_type.c_str(),
                                               &secret[0], secret_len);
  } else {
    g_client_sess.proto = sess_proto;
...
static int proto_inner_callback_on_error(libatgw_inner_v1_c_context, const char *filename, int line, int errcode,
                                         const char *errmsg) {
